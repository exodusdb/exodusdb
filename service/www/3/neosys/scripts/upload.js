...
function* showuploadtable() {
 ...
 if (!(yield* neosyssecurity('UPLOAD CREATE')))
...
function* audiovisual_open(event) {
 ...
 var imagen=event.target.getAttribute('neosysimagen')
...
function* audiovisual_delete(event) {
 ...
 var filename=event.target.getAttribute('neosysfilename')
 
 //confirm!
 if (!(yield* neosysyesno(filename+'\rWarning! Are you SURE that you want to irrevocably delete this file permanently?\r\rNote: This is irreversible!',2))) return yield* neosysinvalid()
...
function* upload_onclick() {
 ...
 if (!sourcefilename) {
  neosyscancelevent()//prevent submit
  return yield* neosysinvalid('Please browse for a file name to upload first')
 }
 ...
 if (sourcefilename.match(/[/<>:"\?\\\*\|]/g)) {
  return yield* neosysinvalid('Sorry but the following characters are not allowed in file names being uploaded\r/ ? < > \\ : * &vert; "')
 }
 
 //check file extensions
 if (!gaudiovisualextensions.neosyslocate(extension))
  return yield* neosysinvalid('Files ending .'+extension+' are not allowed to be uploaded\r\rThe allowed file extensions are:\r\r'+gaudiovisualextensions.join(', '))
 ...
 */
 ...
 //confirm update
 if (mode=='UPDATE') {
  if (!(yield* neosysyesno(question+'Warning! Are you SURE that you want to irrevocably overwrite the existing file permanently?\rThis action cannot be undone!',2)))
    return yield* neosysinvalid()
 } else {
  //if (!(yield* neosysokcancel(question+'\rOK to upload this file now?',1))) return yield* neosysinvalid()
 }
 ...
 //if (!(yield* neosyssecurity('MATERIAL '+mode))) return yield* neosysinvalid(gmsg)
...
function* loadimages() {
 ...
 for (imagen=0;imagen<imageURLs.length;imagen++) {
  ...
  //make it work with url starting with /neosys/ etc ie non-default web site
...
function* adddeletebutton(filename) {
 ...
 //add a delete button
 if (yield* neosyssecurity('UPLOAD DELETE')) {
