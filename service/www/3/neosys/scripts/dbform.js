...
if (typeof gmodule == 'undefined') {
    gmodule = ''
    neosyssettimeout('formfunctions_onload', 10)
}
...
function* formfunctions_onload() {
    ...
    if (typeof form_onload == 'function') {
        if (!(yield* neosysevaluate('yield* form_onload()', 'yield* form_onload()')))
            return
    }
    ...
    //gro = new neosysrecord(yield* neosysevaluate('dict_' + gdictfilename + '(gparameters)', 'yield* formfunctions_onload()'), gdatafilename)
    var dictfunctionname=(guseyield?'yield * ':'') + 'dict_' + gdictfilename
    var dictarray=yield* neosysevaluate(dictfunctionname + '(gparameters)', 'yield* formfunctions_onload()')
    gro = new neosysrecord(dictarray, gdatafilename)
    ...
    while (element = nextelement(element)) {
        ...
        //force cursor on neosystype B for button
        if (element.getAttribute('neosystype') == 'B' && element.style)
        ...
        if (element.getAttribute('neosysgroupno') == null && element.tagName != 'OPTION') {
            ...
            if (!dictitem) {
                ...
                if (datafld)
                    systemerror('yield* formfunctions_onload()','Form element '+neosysquote(fieldname)+' is not in the '+neosysquote(gdictfilename)+' dictionary\nor is not in the correct group.')
                continue
            }
            ...
            if (gKeyNodes){
                ...
                for (var wordn=0;wordn<words.length;++wordn) {
                    if (!(yield* neosyssecurity(gdatafilename.neosyssingular()+' UPDATE '+words.slice(0,wordn+1).join(' ').neosysquote()))) {
            ...
            if (temp && temp != element) {
                ...
                if (temp0.type != 'radio' && temp0.type != 'checkbox') {
                    systemerror('name ' + neosysquote(fieldname) + ' is already in use elsewhere\n' + eval(temp).toString().slice(0, 100) + '\n...')
                    continue
                }
            }
            //}
           ...
           ///log('formfunctions_onload ' + fieldname)
            ...
            if (
                (element.getAttribute('neosysradio') && element.type != 'radio')
                ||
                ...
                for (var ii = 0; ii < newspan.childNodes.length; ii++) {
                    ...
                    if (element.type == elementtype) {
                        ...
                        element.setAttribute('neosysonclick','yield* onclickradiocheckbox()')
                    }
            ...
            var neosysdropdown=element.getAttribute('neosysdropdown')
            if (neosysdropdown) {

                neosysassertobject(element, 'formfunctions_onload', 'element (neosysdropdown)')
                if (typeof neosysdropdown=='string')
                    neosysdropdown=neosysdropdown.split(fm)
                var request = neosysdropdown[0]
                var colarray = neosysdropdown[1]
                var noautoselection = neosysdropdown[2]
                ...
                yield* neosyssetdropdown(selectelement, 'CACHE\r'+request, colarray, '', noautoselection)
            ...
            if (element.getAttribute('neosysconversion')) {
                ...
                if (
                    typeof (element.getAttribute('neosysconversion')) == 'string'
                    &&
                ...
                //conversion is an array of options
                    ...
                    // if (element.id=='USER_CODE') yield* neosysbreak('x '+element.id+' '+element.getAttribute('neosysrequired')+' '+element.outerHTML)
                    var temp = document.createElement('select')
                    copydictitem(dictitem, temp)

                    //if (element.id=='USER_CODE') yield* neosysbreak('y '+temp.id+' '+temp.getAttribute('neosysrequired')+' '+temp.outerHTML)
                    temp.size = 1
                    //element.swapNode(t)
                ...
                if (type == 'F' && !dictitem.allowduplicatefieldno) {
                    ...
                    if (gds.dict.fieldandwordns[fieldandwordn] != element.id) {
                        ...
                        if (typeof gds.dict.fieldandwordns[fieldandwordn] != 'undefined') {
                            ...
                            systemerror('formfunction_onload', neosysquote(name) + ' duplicate dictionary name, field no, word no ' + fieldandwordn + ' is not allowed without .allowduplicatefieldno=true\n' + (gds.dict.fieldandwordns[fieldandwordn]))
                        }
            ...
            if (element.getAttribute('neosystype') == 'F' && element.tagName == 'SPAN' && element.style.display != 'none') {
                ...
                //if (!isMSIE) {
                if (!isMSIE && !element.getAttribute('neosysreadonly')) {
                    ...
                    if (element.getAttribute('neosyspopup') || element.getAttribute('neosyslink')){
                        element.style.display = 'inline-block'
  //                      element.style.float='left'
            ...
            if (
                ...
                if (typeof element.getAttribute('neosyspopup') == 'string'
                        || element.tagName == 'SELECT') {
                    //conversion is a routine eg [yield* neosysfilepopup(filename,cols,coln,sortselect] [popup.clients]
...
//                    element2.style.float='left'
                    ...
                    if (element.getAttribute('neosysalign') != 'T')
...
//                    element2.style.verticalAlign = 'middle'
            ...
            if (element.getAttribute('neosysimage')) {
                var element2 = document.createElement('img')
            ...
            if (
                !element.getAttribute('neosysalign')
                &&
                typeof (element.getAttribute('neosysconversion')) == 'string'
                &&
                element.getAttribute('neosysconversion').indexOf('[NUMBER') >= 0
            ) {
            ...
            if (
                element.getAttribute('neosysalign') == 'R'
                &&
                (
                    groupno > 0
                    ||
            ...
            if (element.tagName.match(gtexttagnames)) {
                if (element.size != 1 && element.getAttribute('neosyslength')) {
                    if (!(parseInt(element.getAttribute('neosyslength')))) {
                        systemerror('yield* formfunctions_onload()', element.id + '.getAttribute("neosyslength")=' + element.getAttribute('neosyslength') + ' is invalid. 10 used.')
...
//                            , autoGrow_minHeight: 100
                        ...
                        var neosysrows=element.getAttribute('neosysrows')
                        if (neosysrows && neosysrows>1)
                            element.rows = neosysrows
                    }
                }
            ...
            if (!(element.getAttribute('neosyslowercase'))) {
                if (element.tagName == 'SELECT'
                        //neosys_dict_text(di) now sets lowercase true but can be removed
            ...
            if (
                !element.getAttribute('neosyslowercase')
                &&
                element.type != 'radio'
                &&
            ...
            if (
                ...
                if (
                    element.getAttribute('neosystype') == 'F'
                    &&
                    !element.getAttribute('neosysreadonly')
                    &&
                    (
            ...
            //element.setAttribute('neosysscreenfn',gfields.length-1)
            //why -1 ???
            element.setAttribute('neosysscreenfn', screenfn)
            //gfields[screenfn].setAttribute('neosysscreenfn',screenfn)
            ...
            //elements with group no - set the data source of repeating lines
                ...
                if (1 && (titleelement = $$(element.id + '_title'))) {
                    ...
                    element2.setAttribute('neosysonclick','yield* sorttable(event)')
                ...
                if (temp=element.getAttribute('neosysallowemptyrows'))
                    tablex.setAttribute('neosysallowemptyrows',temp)

                //any element can determine if a row is required
                if (temp=element.getAttribute('neosysrowrequired'))
                    tablex.setAttribute('neosysrowrequired',temp)

                //need to know the last data entry column
                if (!element.getAttribute('neosysreadonly') && element.tagName.match(gdatatagnames)) {
                    //spans are only input if .isContentEditable
                    if (element.tagName != 'SPAN' || element.isContentEditable) {
                        tablex.setAttribute('neosyslastinputcolscreenfn',element.getAttribute('neosysscreenfn'))
                        //element.setAttribute('neosysislastinputcolumn',true)
                    }
                }
                ...
                if (
                    !tablex.getAttribute('neosysfirstinputcolscreenfn')
                    && !element.getAttribute('neosysreadonly')
                    && element.tagName.match(gdatatagnames)
                ) {
                    //spans are only input if .isContentEditable
                    if (
                        ...
                        tablex.setAttribute('neosysfirstinputcolscreenfn',element.getAttribute('neosysscreenfn'))
                        element.setAttribute('neosysisfirstinputcolumn',true)
                    }
                }
                ...
                if (Number(tablex.getAttribute('neosysgroupno'))) {
                    ...
                    if (Number(tablex.getAttribute('neosysgroupno')) != groupno) {
                        systemerror('yield* formfunctions_onload()', element.name + ' is in group ' + groupno + ' but the table has already been bound to group ' + Number(tablex.getAttribute('neosysgroupno')))
                        return
                    }
                    ...
                    function* maybe_remove_rowbutton(insertdelete) {
                        if (
                            element.getAttribute('neosysno'+insertdelete+'row')
                            && !tablex.getAttribute('no'+insertdelete+'row')
                        ) {
                            tablex.setAttribute('no'+insertdelete+'row',true)
                            neosysremoveelementsbyid(insertdelete+'rowbutton' + groupno)
                        }
                    }

                    //maybe remove insertrow/deleterow buttons (in case first group dictionary element does not have the flag)
                    //if (element.getAttribute('neosysnoinsertrow') && !tablex.getAttribute('noinsertrow')) {
                    //if (element.getAttribute('neosysnodeleterow')&& !tablex.getAttribute('nodeleterow')) {
                    ...
                    //      if (!tablex.className) tablex.className='neosysform'
                    //      tablex.border=1
                    ...
                    if (!(element.getAttribute('neosysnoinsertrow'))) {
                        ...
                        + '" neosysonclick="yield* insertrow_onclick(event)" src="' + ginsertrowimage
                    ...
                    if (!(element.getAttribute('neosysnodeleterow'))) {
                        ...
                        + '" neosysonclick="yield* deleterow_onclick(event)" src ="' + gdeleterowimage
                        + '" style="cursor:pointer">'//: solid 1px">'
                    }
                    t += ' </span>'
                    ...
                    t += ' style=display:none neosysonclick="yield* form_filter(\'unfilter\',' + groupno + ')"'
                    t += '>Show All</button>'
 ...
 || functioncode.indexOf("'" + field2.getAttribute('neosysname') + "'", 0) >= 0) {
                    ...
                    if (
                        Number(field2.getAttribute('neosysgroupno'))
                        &&
    ...
    if (gKeyNodes) {
        ...
        //PRINT
        if (gKeyNodes[0].getAttribute('neosysprintfunction'))
    ...
    gneosys_menubar.insertBefore(temp, gneosys_menubar.firstChild)

    if (gKeyNodes)
        //document.body.insertBefore(formbuttons, document.body.firstChild)
        gneosys_menubar.insertBefore(formbuttons, gneosys_menubar.firstChild)
    else {
...
//alert(formbuttons.parentNode.innerHTML)
    ...
    neosyssetexpression(saverecord, 'style:display', 'saverecord.getAttribute("disabled")?"none":""')
    neosyssetexpression(closerecord, 'style:display', 'closerecord.getAttribute("disabled")?"none":""')

    //program printsend button to be invisible when disabled
    if (printsendrecord)
        neosyssetexpression(printsendrecord, 'style:display', 'printsendrecord.getAttribute("disabled")?"none":""')

    //program new/release/delete buttons to be invisible if disabled
    if (gKeyNodes) {
        neosyssetexpression(newrecord, 'style:display', 'newrecord.getAttribute("disabled")?"none":""')
        neosyssetexpression(editreleaserecord, 'style:display', 'editreleaserecord.getAttribute("disabled")?"none":""')
        if (deleterecord)
            neosyssetexpression(deleterecord, 'style:display', 'deleterecord.getAttribute("disabled")?"none":""')
    }
    ...
    if (openfunction) {
        openrecord.setAttribute('neosyspopup',openfunction)
    ...
    if (typeof form_postinit == 'function') {
        ...
        yield* neosysevaluate('yield* form_postinit()', 'form_functions()')
    ...
    if (gKeyNodes) {
        if (gparameters.key) {
            ...
            //neosyssettimeout('yield* opendoc(' + neosysquote(gparameters.key.replace(/\\/g, '\\\\')) + ')', 1)
...
function* setfirstlastcolumn(groupno) {
    ...
    if (!tablex)
        yield* neosysinvalid('yield* setfirstlastcolumn() table' + groupno + ' is missing')

    for (var ii = 0; ii < gtables[groupno].length; ii++) {
        var screenfn = gtables[groupno][ii]
        //shouldnt this also be restricted to elements with neosysfieldno?
        if (
            !gfields[screenfn].getAttribute('neosysreadonly')
            && neosysenabledandvisible(gfields[screenfn].id)
        )
            tablex.setAttribute('neosyslastinputcolscreenfn',screenfn)
    }
...
function* updatedisplay(elements) {
    ...
    var display = yield* gds.evaluate(element.getAttribute('neosysdisplay'))
...
function* element_neosyssetdropdown(element, request, noautoselection) {

    assertelement(element, 'element_setdropdown', 'element')

    if (!(element.getAttribute('neosysdropdown'))) {
        systemerror('yield* element_neosyssetdropdown()', element.id + ' has no dropdown')
        return
    }
    ...
    yield* neosyssetdropdown(element, request, colarray, '', noautoselection)

}

//be careful this is a sync function without _sync in name so only convert neosys3 to neosys2
//or add this function name to the list of functions that dont require yield if converting neosys2 to neosys3
...
function* newrecordfocus() {
    ...
    if (element)
        ...
        if (gloaded || !gKeyNodes) {
            ...
            if (!element || !element.getAttribute || !Number(element.getAttribute('neosysfieldno'))) {
    ...
    neosyssettimeout('focuson("' + tt + '")', 100)
...
function* tablex_onreadystatechange(event) {
    ...
    //login('tablex_onreadystatechange ' + Number(event.target.getAttribute('neosysgroupno')))

    yield* newrecordfocus()

    //logout('tablex_onreadystatechange ' + Number(event.target.getAttribute('neosysgroupno')))
...
function* printsendrecord_onclick(event) {
    ...
    yield* neosysevaluate(printfunction, 'yield* printsendrecord_onclick()')
    //neosyssettimeout("yield* neosysevaluate('"+printfunction+"','yield* printsendrecord_onclick()')",100)
...
function* listrecord_onclick(event) {
    ...
    yield* neosysevaluate(listfunction, 'yield* listrecord_onclick()')
...
function window_onunload_sync() {
    ...
    if (glocked) {
        ...
        neosyssetcookie(glogincode, 'NEOSYSpending',pending)
...
function* document_onclick(event) {
   ...
   ///log('document_onclick ' + event.target.id)
    ...
    if (event.target.getAttribute('isneosyspopup'))
        result=yield*  neosyspopup(event)

    else if (event.target.getAttribute('isneosyslink'))
        result=yield* neosyslink(event)

    //call the first neosysonclick expression found in element then parents
    var target=event.target
    do {
        var onclickexpression=target.getAttribute('neosysonclick')
    ...
    if (onclickexpression) {
        ...
        //neosysevaluate3 will then be able to arrange that
        // the noclick function can refer to the event variable
        result=yield* neosysevaluate(onclickexpression.replace(/\(\)$/,'(event)'),null,'event',event)
    }
...
*/
   ...
   ///log('document_onkeydown ' + event.target.id + ' ' + keycode)
    ...
    if ((keycode == 75 && event.altKey) || (keycode == 36 && event.altKey)) {
        neosyssettimeout('focuson(gstartelement.id)', 1)
        return neosyscancelevent(event)
    }
    ...
    if (event.altKey && [71, 78, 76, 79, 83, 67, 82, 69, 77, 80, 88].neosyslocate(gkeycode)) {
        neosyscancelevent(event)
        var found = true
        //alt+m main menu
        if (gkeycode == 77) {
            ...
            */
                neosyssettimeout('menuonmouseover(null,$$("menubutton"),13)', 1)
                //menubutton.click()
        }
        ...
        //else if (gkeycode==76) neosyssettimeout('yield* neosyslogout_onclick()',1)
        //Logout and List swapped to be G and L respectively
        else if (gkeycode == 71) yield* neosyslogout_onclick()//g
    ...
    if (keycode == 219 && event.altKey && event.shiftKey) {
        neosyssettimeout('yield* firstrecord_onclick()', 1)
        return neosyscancelevent(event)
    }

    //alt+} is last record
    if (keycode == 221 && event.altKey && event.shiftKey) {
        neosyssettimeout('yield* lastrecord_onclick()', 1)
        return neosyscancelevent(event)
    }

    //alt+[ is previous record
    if (keycode == 219 && event.altKey) {
        neosyssettimeout('yield* previousrecord_onclick()', 1)
        return neosyscancelevent(event)
    }

    //alt+] is next record
    if (keycode == 221 && event.altKey) {
        neosyssettimeout('yield* nextrecord_onclick()', 1)
        return neosyscancelevent(event)
    }

    //alt+^ is select record
    if (keycode ==  54 && event.altKey && event.shiftKey) {
        neosyssettimeout('yield* selectrecord_onclick()', 1)
        return neosyscancelevent(event)
    }

    //alt+end is goto bottom
    if (keycode == 35 && event.altKey) {
        neosyssettimeout('focuson(gfinalinputelement.id)', 1)
        return neosyscancelevent(event)
    }
    ...
    ggroupno = element.getAttribute('neosysgroupno')

    //the rest of the keys are only when located on a neosys data entry field
    if (typeof ggroupno == 'undefined' || ggroupno == null) {
        if (keycode == 37 || keycode == 38) {
            focusprevious(element)
            return neosyscancelevent()
        }
    ...
    if (ggroupno > 0) {
        ...
        if (!rs) {
            neosyscancelevent(event)
            return yield* neosysinvalid('Group number on non-group field')
        }
    ...
    if (tablex && (keycode == 36 || keycode == 35) && event.ctrlKey && !event.shiftKey && !event.altKey) {
        ...
        for (var ii=startrown;ii>=0 && ii<grows.length;ii+=increment) {
            var element=grows[ii].neosysfields[id]
            if (neosysenabledandvisible(element)) {
                focuson(element)
                break
            }
    ...
    if ((ggroupno > 0) && grecn == (rs.length - 1)) {
        if ((keycode == 9 || keycode == 13 || keycode == 40) && !event.shiftKey && !event.ctrlKey && !tablex.getAttribute('noinsertrow')) {
            ...
            //neosysfirstinputcolscreenfn
            var firstcolelement = rowx.neosysfields[gfields[gtables[ggroupno][0]].id]
            if (   (glocked|| !gKeyNodes)
                   //&& element.getAttribute('neosysscreenfn') == tablex.getAttribute('neosyslastinputcolscreenfn')
                   && element.getAttribute('neosysscreenfn') == tablex_lastinputcolscreenfn
                ...
                if (glocked || !gKeyNodes) {
                    ...
                    //var nextelement=$$(gfields[tablex.getAttribute('neosysfirstinputcolscreenfn')].id)[grecn+1]
                    //focuson(nextelement)
                    focusnext()
                    return neosyscancelevent(event)
                }
            ...
            if (element.getAttribute('neosysscreenfn') == tablex_firstinputcolscreenfn && getvalue(element) == '') {
...
*/
        ...
        if (event.keyCode!=keycode) {
            ...
            if (event.shiftKey)
                ...
                if (ggroupno
                    &&element.getAttribute('neosysrequired')
                    &&element.getAttribute('neosysscreenfn')==tablex_firstinputcolscreenfn
                    &&!getvalue(element)
                   ){
                   notgroupno=ggroupno
                }
...
//            alert(selection.isCollapsed + ' : ' + selection.getRangeAt(0).startOffset  + ' : ' +  selection.getRangeAt(0).endOffset + ' : ' + selection.rangeCount + ' : ' + text.length)
    ...
    if ((keycode == 38 || keycode == 40) && ggroupno == 0 && !event.ctrlKey && !event.shiftKey && !event.altKey) {
        ...
        if (element.tagName == 'SELECT' && element.getAttribute('neosysallowcursor'))
            return true
    ...
    if (ggroupno != 0 && rown >= 0 && (keycode == 37 || keycode == 39) && !event.ctrlKey && !event.shiftKey && !event.altKey && element.tagName != 'SELECT') {
        ...
        //if (!element.getAttribute('neosysisfirstinputcolumn') || !element.getAttribute('neosysislastinputcolumn')) {

        //if (!element.getAttribute('neosysisfirstinputcolumn') || element.getAttribute('neosysscreenfn') != tablex.getAttribute('neosyslastinputcolscreenfn')) {
        var screenfn = element.getAttribute('neosysscreenfn')
    ...
    if (keycode == 34 || keycode == 33 || keycode == 40 || keycode == 38 || keycode == 37 || keycode == 39) {
        ...
        if (keycode == 40 && grecn == (grows.length - 1)) {
            ...
            if (element.getAttribute('neosysscreenfn') != tablex_firstinputcolscreenfn) {
                focusnext(element)
                return neosyscancelevent(event)
            }
        ...
        if (keycode == 34 || (keycode == 40 && !event.ctrlKey && !event.shiftKey && !event.altKey)) {
            ...
            if (rown < nrows - 1) {
                if (keycode == 40) {
                    if (!grows[rown + 1].neosysfields[id]) {
                        systemerror('yield* document_onkeydown()', 'Cannot locate id ' + id + ' in rown: ' + rown + ' nrows:' + grows.length + ' tagname:' + grows[rown + 1].neosysfields[id])
                        return false
                    }
                    focuson(grows[rown + 1].neosysfields[id])
                }
                else {
                    var newrown = grecn + gpagenrows
                    if (newrown > (nrows - 1))
                        newrown = nrows - 1
                    focuson(grows[newrown].neosysfields[id])
                }
            }
            ...
            //if on last row go to first row of next page if any, otherwise stay put
                ...
                if (pagen < npages - 1) {
                    //tablex.nextPage()
                    focuson(grows[0].neosysfields[id])
                }
            }
        ...
        if (keycode == 33 || (keycode == 38  && !event.ctrlKey && !event.shiftKey && !event.altKey)) {
            ...
            if (event.ctrlKey) {
                ...
                if (rown > 0) {
                    ...
                    if (keycode == 38) {
                        var newrown=rown-1
                        while (newrown>0 && !neosysenabledandvisible(grows[newrown])) {
                            newrown--
                        }
                        focuson(grows[newrown].neosysfields[id])
                    }
                ...
                //(same for pgup and up)
                    ...
                    if (grecn == 0) {
                        ...
                        //tablex.previousPage()
                        focuson(grows[nrows - 1].neosysfields[id])
                    }
                }
            }
        }
    ...
    if (event.ctrlKey && keycode == 78 && rowx) {
        var button = rowx.neosysfields['insertrowbutton' + ggroupno]
    ...
    if (event.ctrlKey && keycode == 68 && rowx) {
        var button = rowx.neosysfields['deleterowbutton' + ggroupno]
        if (!event.repeat && button && button.style && button.style.display != 'none') {
            neosyscancelevent(event)
    ...
    if (!(event.altKey || event.ctrlKey)
        ...
        if (![9, 16, 17, 18, 20, 35, 36, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123].neosyslocate(keycode)) {
            ...
            if (gKeyNodes && !glocked) {
                ...
                yield* neosysinvalid(element.getAttribute('neosysreadonly'))
            }
            return false
        }
    }
...
//return the first or last enabled and visible input neosysscreenfn given a table element
...
function form_getfirstinputcolscreenfn(tablex,last) {
    var groupno=tablex.getAttribute('neosysgroupno')
    ...
    for (var ii=0;ii<sfns.length;++ii) {
        ...
        if (!element.getAttribute('neosysfieldno') || element.getAttribute('neosysreadonly') || !element.tagName.match(gdatatagnames))
            continue
    ...
    return tablex.getAttribute(last?'neosyslastinputcolscreenfn':'neosysfirstinputcolscreenfn')
}
...
function focusdirection(direction, element, notgroupno, scopex) {
    ...
    var origgroupno=nextelement.getAttribute('neosysgroupno')
    ...
    while (true) {
        ...
        if (notgroupno !== '' && Number(nextelement.getAttribute('neosysgroupno')) == notgroupno) {
            //console.log('SKIP '+nextid+' is group '+notgroupno)
            continue
        }

        //skip readonly
        if (nextelement.getAttribute('neosysreadonly')) {
        ...
        if (isMSIE&&!neosysenabledandvisible(nextelement)) {
    ...
    if (direction<0) {
        var nextgroupno=nextelement.getAttribute('neosysgroupno')
        if (nextgroupno!=origgroupno&&nextgroupno!=0) {
            if (gtables[nextgroupno]) {
                var tablex=gtables[nextgroupno].tablex
                if (tablex) {
                    var tablex_lastinputcolscreenfn=form_getlastinputcolscreenfn(tablex)
                    if (nextelement.getAttribute('neosysscreenfn')==tablex_lastinputcolscreenfn) {
                        var tablex_firstinputcolscreenfn=form_getfirstinputcolscreenfn(tablex)
                        ...
                        setupnewrow(row)//create .neosysfields
                        var firstcolelement=row.neosysfields[firstinputcolid]
                        if (firstcolelement)
...
function* newrecord_onclick() {
    ...
    if (gKeyNodes.length == 1) {
        ...
        var defaultvalue = keyelement.getAttribute('neosysdefaultvalue')
        ...
        if (
            ( !defaultvalue && keyelement.getAttribute('neosysrequired') )
            ||
            ...
            //neosyssettimeout('focuson("' + gKeyNodes[0].id + '")', 200)
            //return true
    ...
    if (gstartelement&&gstartelement.id)
        neosyssettimeout('focuson("' + gstartelement.id + '")', 100)

    neosyssettimeout('focuson("' + tt + '")', 200)
...
function* saverecord_onclick() {
    ...
    if (gKeyNodes) {
        ...
        if (typeof form_postwrite == 'function') {
            //yield* neosysevaluateall('yield* form_postwrite()')
            if (!(yield* neosysevaluateall('yield* form_postwrite()')))
                return false
        }
        //otherwise automatic option to print if available
        else {
            //TODO put this back as a form level opt in
            //   if (printsendrecord&&!printsendrecord.getAttribute('disabled')&&(yield* neosysyesno('Print/Send '+gkeyexternal+' ?',2))) yield* printsendrecord_onclick()
        }
    ...
    if (typeof (form_prewrite) == 'function')
        if (!(yield* neosysevaluateall('yield* form_prewrite()', 'yield* saverecord_onclick()')))
            return false

    //option to confirm
    if (gparameters.confirm || gparameters.savemode && gparameters.savemode.indexOf('CONFIRM') >= 0) {
        if (!(yield* neosysyesno('OK to continue?', 2)))
            return false
    }
    ...
    if (typeof form_write == 'function') {
        ...
        if (!(yield* neosysevaluateall('yield* form_write()', 'yield* saverecord_onclick()')))
            return false
        setchangesmade(false)

        if (typeof (form_postwrite) == 'function')
            yield* neosysevaluateall('yield* form_postwrite(db)', 'yield* saverecord_onclick()')
    ...
    if (window.dialogArguments && (!gparameters.savemode || !gparameters.savemode.neosysswap('CONFIRM', ''))) {
        ...
        //window.close()
        //return false
        return neosyswindowclose(gro.revstr)
    }
...
function* closerecord_onclick() {
    ...
    //if (window.dialogArguments)
    {
        ...
        if (gchangesmade) {
            var response = yield* neosysconfirm('Discard data or instructions entered ?', 1, '', 'D<u>i</u>scard', '<u>C</u>ancel')
        ...
        if (window.dialogArguments) {
            ...
            return neosyswindowclose(returnvalue)
        }
...
function* deleterecord_onclick(event) {
    ...
    if (typeof form_postdelete == 'function') {
        //yield* neosysevaluateall('form_postdelete()')
        if (!(yield* neosysevaluateall('form_postdelete()')))
            return false
    }
...
function* opendoc_body(newkey) {
    ...
    if (newkey && gKeyNodes.length > 1) {
        ...
        if (sepchar && newkey1.split(sepchar).length != gKeyNodes.length) {
            systemerror('opendoc', newkey1.neosysquote() + ' does not have the correct number of key parts ('+gKeyNodes.length+')')
            return false
        }
    }
    ...
    if (!opened) {
        neosyssettimeout('try{gKeyNodes[gKeyNodes.length-1].focus()}catch(e){}', 10)
        return false
    }
...
function* opendoc2(newkey0) {
    ...
    if (typeof (form_preread) == 'function') {
       ...
       ///log('preread external key=' + gkeyexternal + ' internalkey=' + gkey)
        if (!(yield* neosysevaluateall('yield* form_preread()', 'yield* opendoc2()')))
    ...
    if (!gro.data) {
        ...
        return yield* neosysinvalid(gro.response)
    }
    ...
    if (db.response.toUpperCase().slice(0, 16) == 'ERROR: NO RECORD') {
        if (gupdateonlymode) {
            //yield* neosysinvalid(neosysquote(gkeyexternal)+' does not exist.')
            yield* neosysinvalid(neosysquote(yield* getkeyexternal()) + ' does not exist.')
        ...
        if (!glocked) {
            ...
            if (db.response.toUpperCase().indexOf('LOCK NOT AUTHORISED') >= 0) {
                yield* neosysinvalid('Sorry, you are not authorised to create new records in this file.')
            ...
            if (greadonlymode || gupdateonlymode || gpreventcreation) {
                //yield* neosysinvalid('Sorry, '+gkeyexternal+' does not exist and\nyou are not authorised to create new records in this file.')
                yield* neosysinvalid('Sorry, ' + (yield* getkeyexternal()) + ' does not exist and\nyou are not authorised to create new records in this file.')
            ...
            yield* neosysinvalid(neosysquote(gkeyexternal) + ' is being created by ' + lockholder + '.\r\n\r\nYou cannot view or update it until they have finished or cancel.')
            //logout('opendoc2 - cannot create new record because ' + lockholder + ' is creating it.')
            return false
        }
        ...
        if (!(yield* neosysokcancel('Document '+neosysquote(getkeyexternal)+' does not exist. Create a new document ?',2))) {
            ...
            if (db.response.toUpperCase().indexOf('LOCK NOT AUTHORISED') >= 0) {
                ...
                if (greadonlymode) {
                    ...
                    if (db.response.toUpperCase().indexOf('CANNOT LOCK RECORD') >= 0) {
                        if ((yield* neosysconfirm(neosysquote(gkeyexternal) + ' is being updated by ' + lockholder + '.\nOpen for viewing only?', 1, 'Yes', '', 'Cancel')) != 1) {
                        ...
                        if (db.response.toString().slice(0, 2) != 'OK')
                            ...
                            yield* neosysnote(db.response.toString().slice(2))
                    }
                }
            }
        }
    ...
    if (typeof form_postread == 'function') {
        grecn = null
        if (!(yield* neosysevaluateall('yield* form_postread()', 'yield* opendoc2()')))
    ...
    if (typeof form_postdisplay == 'function') {
        grecn = null
        if (!(yield* neosysevaluateall('yield* form_postdisplay()', 'yield* opendoc2()'))) {
            if (glocked)
                yield* unlockdoc()//fail safe
            yield* cleardoc()
    ...
    if (glocked
        ...
        if (typeof form_prewrite != 'undefined' && mode == 'SAVE') {
            ...
            if (gparameters.savemode == 'CONFIRM') {
                if (!(yield* neosysokcancel('OK to save ' + neosysquote(gkeyexternal.neosysconvert('*', ' ')) + ' ?', 1))) {
                    //return false
            ...
            var response = yield* neosysconfirm(action + ' ' + neosysquote(gkeyexternal.neosysconvert('*', ' ')) + ' ?', 1, savetitle, discardtitle, canceltitle)
        }
        ...
        if (window.dialogArguments && (mode == 'SAVE' || mode == 'DELETE' || mode == 'CLOSE')) {
            ...
            //window.returnValue=''
            //return window.close() && false
            return neosyswindowclose(window.returnValue)
...
function* cleardoc() {
    ...
    if (typeof form_postdisplay == 'function') {
        grecn = null
        yield* neosysevaluateall('yield* form_postdisplay()', 'yield* formfunctions_onload()')
    }
...
function* deletedoc() {
    ...
    if (!glocked)
        return yield* neosysinvalid(yield* readonlydocmsg())

    //prevent delete if new record
    if (gds.isnewrecord)
        return yield* neosysinvalid('You cannot delete this document because it hasnt been saved')

    //login('deletedoc')

    var question = neosysquote(gkeyexternal) + '\nWarning! Are you SURE that you want to delete this document?'
    if ((yield* neosysyesno(question, 2)) != 1) {
        yield* neosysinvalid('The document has NOT been deleted\nbecause you did not confirm.')
    ...
    if (db.response != 'OK' && typeof form_postdelete == 'undefined')
        yield* neosyswarning(db.response.slice(2))

    //deleting a record automatically unlocks it
    glocked = false

    yield* neosyswarning(neosysquote(gkeyexternal) + ' has been deleted.')
    ...
    if (window.dialogArguments) {
        ...
        //return window.close()
        return neosyswindowclose('DELETE ' + savekey)
...
function* form_oninput(event) {
    ...
    var fn=Number(event.target.getAttribute('neosysfieldno'))
    if (!fn)
        return true
...
function* validateall(mode) {
    ...
    for (groupno = 0; groupno == 0 || groupno <= gmaxgroupno; groupno++) {
        ...
        var allowemptyrows = (groupno == 0) ? false : tablex.tableelement.getAttribute('neosysallowemptyrows')
        var rowrequired = (groupno == 0) ? false : tablex.tableelement.getAttribute('neosysrowrequired')
        var nrowsfound = 0
        ...
        for (var rown = 0; rown < rows.length; rown++) {
            ...
            if (missingelement && (!allowemptyrows || anydata)) {
                yield* neosysinvalid(missingelement.getAttribute('neosystitle') + ' is required.')
            ...
            if (mode != 'filldefaults'
                ...
                if (!allowemptyrows) {
                    ...
                    yield* neosysinvalid('Empty rows are not allowed for ' + missingelement.getAttribute('neosystitle'))
                    focuson(missingelement)
        ...
        if
          (
           ...
           && neosysenabledandvisible($$('neosysgroup' + groupno))
            ...
            var missingelement = rows[0][firstcolumnname].element
            yield* neosysinvalid('At least one ' + missingelement.getAttribute('neosystitle') + ' is required.')
            focuson(missingelement)
...
function* writedoc(unlock) {
    ...
    gvalue = ''
    if (typeof (form_prewrite) == 'function') {
        if (!(yield* neosysevaluateall('yield* form_prewrite()', 'yield* writedoc()'))) return false
    ...
    if (!(/**/ yield * gro.writex(unlock))) {
        ...
        //yield* neosysnote('Cannot save '+neosysquote(gkeyexternal)+' because: \r\r'+gro.response)
        yield* neosysinvalid('Cannot save ' + neosysquote(gkeyexternal) + ' because: \n\n' + gro.response)
    ...
    if (gro.response != 'OK' && typeof form_postwrite == 'undefined') {
        yield* neosyswarning(gro.response.slice(2))
    }
...
function startrelocker() {
    ...
    grelocker = neosyssetinterval('yield* relockdoc()', glocktimeoutinmins / 2.2 * 60 * 1000)
}
...
function* relockdoc() {
    ...
    if (yield* relockdb.send()) {
        ...
        if (response.toUpperCase().indexOf('EXPIRED') >= 0) {
            ...
            setgraphicbutton(editreleaserecord, '<u>E</u>dit', geditimage)
            setdisabledandhidden(deleterecord, true)
            yield* neosyswarning(response)
        }
        else {
            yield* neosysnote(response)
        }
...
function* unlockdoc() {
    ...
    if (glocked) {
        while (true) {
            ...
            if (yield* db.send())
                break
            if (!(yield* neosysyesno('Cannot release document - try again?\n\n' + db.response))) break
        }
    }
...
function focuson(element) {
    ...
    if (typeof (element) == 'string') {
        ...
        if (!element2)
            return false //logout('focuson - cannot getElementById ' + neosysquote(element))
...
function focuson2() {
    ...
    //allowreadonly=true
    if (!(neosysenabledandvisible(focusonelement,true)))
...
function* document_onfocus(event) {
   ...
   ///log(' fill in any defaults where possible')
    if (!gKeyNodes || glocked)//check keynodes as well now
    {
       ...
       ///log('check no missing data in group 0 always')
        ...
        // if (!(yield* checkrequired(elements,element,Number(element.getAttribute('neosysgroupno')))))
   ...
   ///log('if the key is complete and not on a key field, get the record')
    ...
    if (gKeyNodes && element != gpreviouselement) {
        ...
        {
            //neosyssettimeout('yield* opendoc()',100)
            yield* opendoc(nextkey)
            return false //logout('document_onfocus' + ' ' + neosysquote(elementid) + ' new record')
        }
    }
...
function* validateupdate() {
    ...
    //if (!gpreviouselement.getAttribute('neosyslowercase') && gpreviouselement != 'radio' && gpreviouselement.type != 'checkbox')
   ...
   //log('User/setdefault changed ' + id + '\nfrom ' + neosysquote(gpreviousvalue) + '\nto ' + neosysquote(newvalue))
    ...
    if (Number(gpreviouselement.getAttribute('neosysgroupno'))) {
        ...
        if (!(yield* checkrequired(elements, gpreviouselement, Number(gpreviouselement.getAttribute('neosysgroupno'))))) {
            setvalue(gpreviouselement, gpreviousvalue)
...
/* cant do this here otherwise will retrieve multipart key records when only clearing
    ...
    if (gKeyNodes
     ...
     && nextkey != gkey) {
        ...
        //neosyssettimeout('yield* opendoc("'+nextkey+'")',1)
...
*/
    ...
    if (gautofitwindow)
        neosyssettimeout('neosysautofitwindow()', 1)
...
function* checkrequired(elements, element, groupno) {
    ...
    for (var ii = 0; ii < elements.length; ii++) {
        ...
        //if (element2.getAttribute('neosysscreenfn')==element.getAttribute('neosysscreenfn'))
        //check ids because neosysscreenfn is repeated in gfields for radio 4,4,4
        ...
        if (Number(element2.getAttribute('neosysgroupno')) == groupno) {
            if (((!foundelement && element2.tabIndex <= element.tabIndex) || (element2.tabIndex != -1 && element2.tabIndex < element.tabIndex))) {
                ...
                if (element
                    ...
                    if (!(yield* setdefault(element2)) && neosysenabledandvisible(element2)) {
                        focuson(element2)
                        return false
                    }
                    ...
                    if (element2.getAttribute('neosysrequired') && !element2.getAttribute('neosysreadonly') && gds.getcells(element2,grecn)[0].text == '') {
                        ...
                        if (element2.disabled || element2.getAttribute('disabled') || !neosysenabledandvisible(element2))
                            return true

                        //put up a message unless is the first column of a row
                        if (true || !(element2.getAttribute('neosysisfirstinputcolumn')))
                            yield* neosysinvalid(element2.getAttribute('neosystitle') + ' is required..')
...
function getvalue(element, recn) {
    ...
    if (typeof (element) == 'string') {
        ...
        if (!element.tagName) {
            //element = element[0]
            if (element[0].getAttribute('neosysgroupno')) {
    ...
    switch (element.tagName) {
        ...
        case 'INPUT': {
                ...
                switch (element.type) {
                    ...
                    case 'text':
                        var tx=element.value.neosystrimr()
                    ...
                    default: {
                            ...
                            return systemerror('getvalue()', neosysquote(element.type) + ' invalid INPUT element type')
                        }
        ...
        default: {
                ...
                return systemerror('getvalue()', neosysquote(element.tagName) + ' invalid tagName')
            }
    }
...
function neosyssetreadonly(elements, msg, options, recn) {
    ...
    if (!(isNaN(+elementx))) {
        ...
        for (var ii = 0; ii < gfields.length; ii++) {
            if (Number(gfields[ii].getAttribute('neosysgroupno')) == elementx
                ...
                && (!(neosyssetreadonly(gfields[ii].id, msg, options, recn))))
                    return false
        }
        return true
    }
    ...
    if (typeof elementx == 'string') {
        ...
        if (!elementx) {
            ...
            if (!(gds.dictitem(elementxstring)))
                return systemerror('neosyssetreadonly()', neosysquote(elements) + ' is not in the form')
            return false
        }
        ...
        if (typeof recn == 'object' && recn.length && !elementx.tagName) {
            ...
            for (var ii = 0; ii < recn.length; ii++) {
                if (!(neosyssetreadonly(elementx[recn[ii]], msg, options)))
                    return false
            }
            return true
        }
    ...
    if (gevent && typeof gevent.target != 'undefined' && gevent.target.getAttribute && gevent.target.getAttribute('neosysreadonly') && gpreviouselement && document.activeElement.tabIndex == 999)
...
function* readonly_onchange(event) {
    ...
    yield* neosysinvalid(readonlymsg)
...
function setvalue2(element, value) {
    ...
    switch (element.tagName) {
        ...
        default: {
                ...
                return systemerror('setvalue2()', neosysquote(element) + ' ' + neosysquote(element.tagName) + ' invalid tagName in setvalue2(' + element + ',' + value + ')')
            }
...
function getradiocheckboxelements(element) {
    ...
    if (document.getElementsByClassName)
        elements = scope.getElementsByClassName('neosysid_' + (element.id ? element.id : element))
    else {
...
function* getdefault(element) {
    ...
    defaultvalueexpression=element.getAttribute('neosysdefaultvalue')
    ...
    //calculate default
    var defaultvalue = yield* neosysevaluate(defaultvalueexpression, 'yield* getdefault(' + element.id + ')')
    ...
    if (typeof defaultvalue != 'string') {
        yield* neosysinvalid(element.id + ' default returned is ' + typeof defaultvalue + ' - "" used\nExpression:'+defaultvalueexpression)
        defaultvalue = ''
    }
...
function* setdefault(element, donotupdate) {
    ...
    element.setAttribute('neosysdefault',gdefault)

    //set the value (externally only)
    //yield* gds.setx(element,grecn,gdefault)

    setvalue(element, yield* oconvertvalue(gdefault, element.getAttribute('neosysconversion')))
...
function* validate(element) {
   ...
   //log('grecn=' + grecn)
    ...
    var elementtitle=element.getAttribute('neosystitle')
    if (!elementtitle)
    ...
    if (element.tagName != 'SELECT' && element.type != 'radio') {
        ...
        */
        ...
        if (Number(element.getAttribute('neosysfieldno'))==0) {
        ...
        if (element.type == 'checkbox')
            invalidcharacters = invalidcharacters.neosysconvert(sm, '')
        if (element.getAttribute('neosysinvalidcharacters')) {
        ...
        if (temp != gvalue) {
            ...
            charsx = invalidcharacters.neosysconvert(charsx, '').neosysswap('|','&#124;')
            //.neosysswap(fm,'&u'+fm.charCodeAt(0)+';')
            yield* neosysinvalid('The following characters are not allowed in '+elementtitle+'.\n\n&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="border:1px solid #DDDDDD;padding-bottom:2px"> ' + charsx.neosysswap(' ', ' space ')+'&nbsp;</span><br />&nbsp;')
        ...
        if (element.getAttribute('neosysvalidcharacters')) {
            ...
            if (temp != '') {
                yield* neosysinvalid('Only the following characters are allowed in '+elementtitle+'.\n\&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"' + element.getAttribute('neosysvalidcharacters').neosysswap('|','&#124;') + '\"')
                return false //logout('validate')
            }
        }
   ...
   //log('required check')
    if (gvalue == ''
         && element.getAttribute('neosysrequired')
         && neosysenabledandvisible(element)) {
        yield* neosysinvalid(elementtitle + ' is required...')
        return false //logout('validate')
    }
   ...
   //log('before conversion')
    ...
    if (conversion && gvalue != '') {
        ...
        gmsg = ''
        ivalue = yield* neosysevaluate(expression, 'yield* validate(' + element.id + ') iconv')
        ...
        if (gvalue == null || ivalue == null) {
            ...
            yield* neosysinvalid(neosysquote(gvalue) + ' is not a valid ' + convarray[0].toLowerCase().replace(/_/g, ' ') + '\n\n' + gmsg)
   ...
   //log('before custom validation')
    ...
    if (elementvalidation) {
        ...
        if (typeof elementvalidation == 'function')
            ...
            ok = yield* neosysevaluate(elementvalidation, 'yield* validate() functioncode')

        if (gvalue == null)
            yield* neosyswarning(element.id + ' validation routine returned gvalue=null')
        if (!ok || gvalue == null) {
            grecn = storegrecn
            yield* neosysinvalid()
    ...
    if (gvalue && ggroupno > 0 && element.getAttribute('neosysunique')) {
        ...
        if (ln = othervalues.neosyslocate(gvalue)) {
            ...
            if (element.getAttribute('neosysnonuniquewarning')) {
                ...
                return yield* neosysinvalid(gmsg)
            }
        }
    }
    ...
    if (gvalue
        ...
        if (elementsequence == 'A') {
            ...
            if ((temp = yield* getpreviousrow('', true, true))
                ...
                return yield* neosysinvalid(title + ' cannot be less than ' + title + ' in the previous row above')
            }
            var temp
            if ((temp = yield* getnextrow('', true, true))
                ...
                //    alert(typeof gvalue+' '+gvalue+' > '+typeof temp+' '+temp)
                //logout('validate - not sequential')
                return yield* neosysinvalid(title + ' cannot be greater than ' + title + ' in the next row below')
            }
        }
    }
...
function* validateoconv(element, ivalue) {
    ...
    gmsg = ''
    var ovalue = yield* neosysevaluate(expression)
    if (typeof ovalue == 'undefined')
        return false

    //null means failed to convert to external value therefore invalid
    if (ovalue == null) {
        yield* neosysinvalid(neosysquote(ivalue) + ' is not a valid ' + convarray[0].toLowerCase() + '\n' + gmsg)
        return false
    }
...
function* neosysevaluateall(functioncode, callerfunctionname) {
    var storegrecn = grecn
    grecn = null
    var result = yield* neosysevaluate(functioncode, callerfunctionname)
    grecn = storegrecn
    return result
}

function* neosysevaluate(functionorcode, callerfunctionname, arg1name, arg1, thisobject) {
    ...
    if (typeof functionorcode == 'undefined') {
        systemerror('yield* neosysevaluate()', 'The required argument "functionorcode" is missing. Called from\n' + callerfunctionname)
    ...
    if (gstepping || gusername == 'NEOSYS' || gusername == 'STEVE')
        result=yield* neosysevaluate3(functionorcode, null, arg1name, arg1, thisobject)
    else {
        ...
        //} if (e) {
            ...
            systemerror('yield* neosysevaluate()'+functionorcode, e)
            return yield* neosysinvalid()
        }
    ...
    //    yield* neosysinvalid('Error in' + '\n' + functioncode.toString().substr(0, 500))
    //    systemerror('yield* neosysevaluate()', 'Function returned "undefined"\nCalled from ' + callerfunctionname + '\n\n' + functioncode.toString().substr(0, 500))
    //    result = ''
    //}
...
function* neosysevaluate3(functionorcode, functionname, arg1name, arg1, thisobject) {

    //arg1name and arg1 are 'event' and event in some use cases
    //in order to pass event into neosysonclick functions
    if (!arg1name)
    ...
    if (functioncode.match&&functioncode.match(gyieldregex)) {
        if (guseyield) {
            try{
                ...
                return systemerror('yield* neosysevaluate3()\nError creating function \n'+functioncode,e)
            }
            if (gcatcherrors) {
                try {
...
function* form_deleterow(event, element) {
    ...
    if (typeof predeleterow == 'function') {
        //if (!(yield* neosysevaluate('yield * form_predeleterow' + groupno + '()')))
    ...
    if (document.getElementsByClassName)
        element = grows[rown].getElementsByClassName('neosysid_' + id)[0]
    else
    ...
    if (typeof postdeleterow == 'function') {
        yield* neosysevaluate('yield * form_postdeleterow' + groupno + '()')
    }
...
function* deleterows(groupnoorelement, rowns) {
    ...
    if (typeof groupno == 'object') {
        groupno = groupno.getAttribute('neosysgroupno')
        if (!groupno && groupnoorelement[0] && groupnoorelement[0].getAttribute)
            groupno = groupnoorelement[0].getAttribute('neosysgroupno')
        if (!groupno && groupnoorelement.parentNode.getAttribute)
            groupno = groupnoorelementparenNode.getAttribute('neosysgroupno')
    }
...
function* readonlydocmsg() {
    ...
    if (gKeyNodes || gkey)
        return yield* neosysinvalid('This document is currently "read only"')

    //readonly before key has been entered
    else if (!gkey)
        return yield* neosysinvalid('Please open a document first')
}
...
function* form_insertrow(event, append) {
    ...
    if (typeof append == 'undefined') {
        ...
        //clicking a single row OR clicking the penultimate row
            ...
            var choice=yield* neosysconfirm('Insert row before or after?',1,'Before','After','Cancel')
            if (!choice)
                return false
            if (choice == 2)
                grecn++//AFTER/BELOW
    ...
    if (typeof preinsertrow == 'function') {
        if (!(yield* neosysevaluate('yield * form_preinsertrow' + groupno + '()')))
    ...
    if (typeof postinsertrow == 'function')
        yield* neosysevaluate('yield * form_postinsertrow' + groupno + '()')
...
function* openrecord_onclick() {
    ...
    if ((reply = yield* neosyspopup2(openrecord)) == null)
...
function* nextrecord2(event, direction) {
    ...
    if (direction == 0) {
        var selkeys = yield* neosysdecide('', gkeys, [[0, 'Key']], 0, '', many = true)
...
function* neosyslink(event, element) {
    ...
    if (!element)
        element = event.target
    while (element && element.getAttribute && !element.getAttribute('neosyslink')) {
        element = element.nextSibling
    }
    if (!element || !element.getAttribute || !element.getAttribute('neosyslink')) {
        var element = event.target
        while (element && (!element.getAttribute || !element.getAttribute('neosyslink'))) {
    ...
    if (!element || !element.getAttribute('neosyslink'))
        return false //logout('neosyslink - no link')
    ...
    if (gKeyNodes && !gloaded && element.getAttribute('neosysfieldno') != 0) {
        focuson(gKeyNodes[0])
        return false //logout('neosyslink - no record')
    }
    ...
    var reply = yield* neosysevaluate(element.getAttribute('neosyslink'), 'yield* neosyslink()')
...
function* neosyspopup(event, element) {
    ...
    if (!element) {
        element = event.target
        while (element
                && ( !element.getAttribute || !element.getAttribute('neosyspopup') )
                && element.tagName != 'SELECT') {
            element = element.nextSibling
        }
        if (!element) {
            ...
            for (var elementn=0;elementn<elements.length;++elementn) {
                ...
                if (element.getAttribute&&element.getAttribute('neosyspopup'))
                 break
            }
        }
    }
    ...
    //quit if no element
    if (!element)
        return false //logout('neosyspopup - no element')

   //log('check no missing data in group 0 always')
    if (!(yield* checkrequired(gfields, element, 0)))
        return false //logout('neosyspopup' + ' ' + element.id + ' a prior element is visible and required but is blank (1)')

    //validateupdate previous field
    //if (element.id!=gpreviouselement.id)
    if (element != gpreviouselement) {
        if (!(yield* validateupdate()))
            return false //logout('neosyspopup - validateupdate failed')
    }

    //cannot update anything but key field if not locked or save button not enabled
    if (element.getAttribute('neosysfieldno') != 0 && gKeyNodes && (!glocked || saverecord.getAttribute('disabled'))) {
        yield* readonlydocmsg()
        return false //logout('neosyspopup - read only document')
    }
    ...
    if (readonly) {
        if (readonly != 'true')
            yield* neosysinvalid(element.getAttribute('neosysreadonly'))
        return false //logout('neosyspopup - read only')
    }

    //quit if no popup defined
    if (!element.getAttribute('neosyspopup') && element.tagName != 'SELECT')
        return false //logout('neosyspopup - no popup')

    //prevent popups except on the key field unless a record is present
    if (gKeyNodes && !gloaded && element.getAttribute('neosysfieldno') != 0) {
        focuson(gKeyNodes[0])
        return false //logout('neosyspopup - no record')
    }
    ...
    //get a reply or return false
    /////////////////////////////
    if ((reply = yield* neosyspopup2(element)) == null) {
//    alert('xxx')
        focuson(element)
        return false //logout('neosyspopup - no reply')
    }
    ...
    if (reply.length > 1
        ...
        if (grecn > 0) {
            ...
            if (!reply.length)
                return false //logout('neosyspopup - no new selections')
        }
    ...
    if (reply == gpreviousvalue) {
        focusnext()
        return false //logout('neosyspopup - no change')
    }

    //do not change key if user chooses not to unload an existing document
    if (element.getAttribute('neosysfieldno') == 0 && gloaded && !(yield* closedoc('OPEN')))
        return false //logout('neosyspopup - user cancelled unloaddoc')

    //output convert it
    if (element.getAttribute('neosysconversion')) {
        reply = yield* validateoconv(element, reply, reply)
        if (typeof reply == 'undefined' || reply == null) {
            return false //logout('neosyspopup - oconv failed')
        }
    }
...
function* neosyspopup2(element) {
    ...
    if (expression) {
        var reply = yield* neosysevaluate(expression, 'yield* neosyspopup2()')
    }
    ...
    //otherwise build a list and select from the SELECT
        ...
        var elgroupno=Number(element.getAttribute('neosysgroupno'))
        if (elgroupno) {
            var tableelement = gtables[elgroupno].tableelement
            var multipleselection = tableelement && tableelement.getAttribute('neosyslastinputcolscreenfn') == tableelement.getAttribute('neosysfirstinputcolscreenfn')
        }

        //get the response(s)
        reply = yield* neosysdecide2('', selectvalues, '1', '', '', multipleselection)
    ...
    if (reply && ((openrecord && element == openrecord) || (element.getAttribute('neosysfieldno') == 0 && gKeyNodes.length == 1))) {
        if (typeof reply == 'object') {
            ...
            if (reply.length > 1 && reply.length <= 50) {
                var openall = 2
                openall=yield* neosysconfirm('Open all in one tab?', 1, 'One', 'Many')
                if (!openall)
                    return false
                if (openall == 2) {
...
function getrecn(element) {
    ...
    var recn
    if (element.getAttribute&&!(Number(element.getAttribute('neosysgroupno')))) {
        recn = null
    }
    else {
        var rowx = getancestor(element, 'TR')
...
function* form_filter(mode, colidorgroupno, regexp, maxrecn, elem) {
    ...
    if (typeof colidorgroupno != 'number' && (mode == 'filter' || mode == 'unfilter')) {
        ...
        //if (!dictitem)
        // return yield* neosysinvalid(colid+' dictitem does not exist in yield* form_filter()')
...
//function* prefilter(){}
    ...
    if (typeof form_prefilter == 'function') {
        //yield* neosysevaluate('form_prefilter()','yield* formfunctions_onload()')
    ...
    var tablex = $$('neosysgroup' + groupno)
    if (!tablex)
        return yield* neosysinvalid(colid + ' is not part of a table')
...
function* form_pop_calendar() {
    ...
    neosyssettimeout('yield* form_popcalendar2()', 100)
    return false
}
...
function* form_popcalendar2() {
    ...
    */
    ...
    function calendar_checkInDatePicker_onchange() {
        ...
        //clear the object
        neosyssettimeout('calendar_checkInDatePicker=null', 1)
    ...
    //xyz=neosyssetinterval('window.status=document.activeElement.outerHTML',10)
    return false
...
function* form_closepopups() {
    ...
    if (calendar_checkInDatePicker) {
        try {
            ...
            if (gusername == 'NEOSYS')
                yield* neosysnote('couldnt drop calendar\n' + e.description)
        }
...
function* form_val_index(filename, fieldname, allownew) {
    ...
    for (var ii = 0; ii < indexvalues.length; ii++) {
        if (indexvalues[ii][fieldname].text.toUpperCase() == gvalue.toUpperCase()) {
            if (!(reply = yield* neosysconfirm('Change the capitalisation of your entry?\n\nFrom: ' + gvalue.neosysquote() + '\n\n  To: ' + indexvalues[ii][fieldname].text.neosysquote(), 1)))
                return yield* neosysinvalid()
            if (reply == 1)
    ...
    if (!allownew)
        return yield* neosysinvalid(gvalue + ' ' + fieldname.toLowerCase() + ' does not exist')

    //confirm is new index value
    if (gvalue && ((yield* neosysyesno(' Is ' + neosysquote(gvalue) + ' to be a new ' + fieldname.toLowerCase() + ' for ' + filename.toLowerCase() + ' ?', 1)) != 1))
        return false
...
function* form_pop_index(filename, fieldname, many) {
    ...
    if (!indexvalues || indexvalues.group1.length == 0)
        return yield* neosysinvalid('No ' + fieldname.toLowerCase().neosysconvert('_',' ') +'(s) have been entered on ' + filename.toLowerCase().neosysconvert('._','  ') + ' file yet')

    //user selects index value(s)
    var result = yield* neosysdecide2('', indexvalues, [[fieldname, fieldname.neosyscapitalise()]], 0, '', many)
...
function* copyrecord_onclick() {
    ...
    if (!gkey || !glastkey || !gds.isnewrecord || gchangesmade)
        return yield* neosysinvalid('To copy a record you must:\n\n1. Open the record to copy\n2. Start a new record&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n3. Click the Copy button&nbsp;&nbsp;&nbsp;&nbsp;')

    //read the record to be copied
    var copyrecord = []
    if (!(yield* copyrecord.neosysread(gdatafilename, glastkey)))
        return yield* neosysinvalid(copyrecord.neosysresponse)
    ...
    //validate copy
    //if (!(yield* neosysevaluate('yield* form_copyrecord()','yield* copyrecord_onclick()')))
...
function form_setdefault(str) {
    gro.defaultrevstr=unescape(str.neosysconvert('`^]}\~',rm+fm+vm+sm+tm+stm))
    return 'ok'
}
...
function* form_postread_noteifdeleted(descending) {
    ...
    var note='This record was deleted by '+(yield* gds.get1('USERNAME_UPDATED',logn))+' on '+(yield* gds.get1('DATETIME_UPDATED',logn)).neosysoconv('[DATE_TIME]')+'.'
    note+='\n\nYou can restore it by saving it again'
    yield* neosysnote(note)
...
function* form_oncopy_generic(event) {
    ...
    var elements = document.getElementsByClassName('neosysid_' + elementid)
    var lns=[]
        ...
        for (var coln=0;coln<ncols;++coln) {
            ...
            if (!dictid)
                ...
                if (colpars[2]&&colpars[2].slice(0,1)=='[')
                    cell=cell.neosysoconv(colpars[2])
                txln.push(cell)
            }
        }
...
function* document_onpaste(event) {
    ...
    if (!element.getAttribute('neosysisfirstinputcolumn')) {
    ...
    var msg=element.getAttribute('neosysreadonly')
    if (msg) {
        neosyscancelevent()
        return yield* neosysinvalid(msg)
    }
...
function form_copypaste_getcols(event, pasting) {
    ...
    if (!tablex)
        return false
    var groupn=tablex.getAttribute('neosysgroupno')
    if (groupn<1)
        return false

    //only copy/paste on rows - get groupn or quit
    //var groupn=element.getAttribute('neosysgroupno')
    //if (groupn<1)
    //    return false
    ...
    for (var fn=0;fn<tablefieldns.length;++fn) {
        ...
        if (pasting && element.getAttribute('neosysreadonly'))
            continue
        var elementtitle=element.id.neosysconvert('_',' ')
        var conversion=element.getAttribute('neosysconversion')
...
//    //user pastes data
//    if (!data) {
//        //args.SCHEDULE_TEXT = gscheduleimportdata.neosysconvert(tm,vm)
//        var args = {}
//        data = yield* neosysshowmodaldialog('../media/schedules_import.htm', args)
//        if (!data)
//            return true
...
//    }
    ...
    //eg "30""" becomes 30"
    //data = data.neosysswap('""', '&quote;').neosysconvert('"', '').neosysswap('&quote;', '"')
    ...
    for (var ln = nlines-1; ln>=0; --ln) {
        ...
        //unfortunately array.neosystrim removes empty cells
        //line=line.neosystrim()
        for (var ii=line.length-1;ii>=0;--ii)
            line[ii]=line[ii].neosystrim()
    ...
    if (nlines < data.startln + 1) {
        ...
        yield* neosysinvalid('Not enough lines to import')
        return true
    }
    ...
    if (data.length<=1) {
        yield* neosysinvalid('Nothing to import')
        return true
    }
    
    //option to quit if any invalid items
    if (ninvalid) {
        if (!(yield* neosysyesno('There were '+ninvalid+' invalid items\nDo you want to continue?\n\n(Invalid items will be imported as blank)')))
            return true
    }
...
function* form_onpaste_ignore_cancel(rown,coln,coltitle,data,msg) {
    return yield* neosysokcancel('WARNING! in Row:'+(rown+1)+' Col:'+(coln+1)+'\n\n'+coltitle+' = "'+data+'"\n\n'+msg,'Ignore','Cancel')
}
...
function* form_onpaste_generic_validatedata(data) {
    ...
    for (var coln=0;coln<ncols;++coln) {
        ...
        col.conversion=element0.getAttribute('neosysconversion')
        if (!col.conversion)
            col.conversion=element0.getAttribute('neosyscheckbox')
        if (col.conversion && col.conversion.slice(0,1)!='[')
            col.conversion=col.conversion.neosyssplit(':;')
            
        col.filename=element0.getAttribute('neosysfilename')
        col.validation=element0.getAttribute('neosysvalidation')
    }
    ...
    if (!nimportablecols) {
        ...
        return yield* neosysinvalid('No recognisable columns in data\n\nPossible columns: '+allcoltx.join(', '))
    }
            ...
            if (datacoln) {
                ...
                if (col.conversion) {
                    ...
                    if (col.conversion.slice(0,1)=='[') {
                        ivalue=ovalue.neosysiconv(col.conversion)
                    }
                    //things like dropdowns and checkboxes
                        ...
                        if (ivalue==null) {
                            gmsg='Allowable values are '+col.conversion.neosysjoin(',=')
                        }
                    }
                ...
                if (col.filename) {
                    ...
                    var rec=[]
                    if (!(yield* rec.neosysread(col.filename,ivalue))) {
                        //'Format must be '+conversion.slice(1,-1).split(',')[0]
