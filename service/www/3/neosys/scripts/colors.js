...
function* colors_val_screencolor() {
    if (!(yield* neosys_val_color())) return yield* neosysinvalid()
    neosys_set_style('screencolor', gvalue)
    return true
}

function* colors_val_screenfont() {
    if (!(yield* neosys_val_font())) return yield* neosysinvalid()
    neosys_set_style('screenfont', gvalue, yield* gds.getx('SCREEN_FONT_SIZE'))
    return true
}

function* colors_val_screenfontsize() {
    neosys_set_style('screenfont', yield* gds.getx('SCREEN_FONT'), gvalue)
    return true
}
...
function neosys_dict_font(di) {
    ...
    di.popup = 'yield* neosys_pop_font('
...
function* neosys_pop_font(required, many) {
    ...
    return yield* neosysdecide2('', tt, [[2, '']], 0, '', many)
...
function neosys_dict_color(di) {
    ...
    di.popup = 'yield* neosys_pop_color('
    if (di.required) di.popup += 'true'
    di.popup += ')'
...
function* neosys_pop_color(required, many) {
    ...
    return yield* neosysdecide2('', tt, [[2, ''], [1, ''], [0, '']], 1, '', many)
...
function neosys_dict_colorfontsize(dict, fn) {
    ...
    //di.defaultvalue='100'
    neosys_dict_number(di, 0, 50, 200)
    di.length = 3
    di.allowcursor = true
