...
function* formfunctions_onload() {
    ...
    if (gisdialog) {
        ...
        datasetcode = neosysgetcookie2('dataset', 'NEOSYS', '')
        ...
        yield* neosyssetdropdown(gdataset_element, "GETDATASETS\r" + gsystem, Array("code", "name"), datasetcode, null)
        if (startinglocation != 'login' && neosysgetcookie2('a', 'NEOSYS', '') == 'true') {
            gusername_element.value = neosysgetcookie2('u', 'NEOSYS', '')
            gpassword_element.value = neosysgetcookie2('p', 'NEOSYS', '')
        ...
        //if (gautologin_element.checked!=true) neosyssetcookie('','NEOSYS','','ll',true)
    }

    gwaitdiv.style.display = "none"

    neosyssettimeout('gusername_element.disabled?gpassword_element.focus():gusername_element.focus()', 100)
...
function* passwordreset_onclick() {
    ...
    //yield* neosysshowmodaldialog(NEOSYSlocation+'passwordreset.htm')
    var usercode = gusername_element.value.toUpperCase()
    if (!usercode) {
        neosyssettimeout('gusername_element.focus()', 100)
        gusername_element.focus()
        gusername_element.select()
        return yield* neosysinvalid('Username is required')
    }
    var oldpass = gpassword_element.value
    var email = yield* neosysinput('Password reset for database "' + gdataset_element.value + '"\n\nWhat is your registered email address?')
    if (!email)
        return
    if (email.indexOf('@') < 0)
        return yield* neosysinvalid('Email address must contain an @ character')
...
function* document_onkeydown(event) {
    ...
    if (gkeycode == 13 || gkeycode == 120) {
        neosyssettimeout('yield* dblogin()', 10)
        //yield* dblogin()
...
function* dblogin() {
    ...
    //glogin_button.click()
    neosyssettimeout('glogin_button.click()',100)
...
function* login_onclick() {
    ...
    var datasetx = neosysgetdropdown(gdataset_element)
    ...
    if (gisdialog) {
        ...
        //return window.close()
        return neosyswindowclose(response)
    }
    ...
    while (true) {
        ...
        if (yield* db.send()) {
            ...
            if (db.response != 'OK') {
                yield* neosyswarning(db.response.slice(3))
            }
            ...
            if (startinglocation == 'login')
                ...
                startinglocation = unescape(neosysgetcookie2('ll', 'NEOSYS', ''))
            ...
            if (!startinglocation || (window.event && window.event.shiftKey)) {
                ...
                if (neosysgetcookie2('m').split(',')[0] == 'TIMESHEETS')
        ...
        if (db.response.indexOf('Password Reset ') >= 0) {
            if (db.response.indexOf('Password Reset Failed') >= 0)
                ...
                return yield* neosysnote(db.response)
        }
        ...
        if (db.response.indexOf('This computer is number') >= 0) {
            ...
            if (db.response.toLowerCase().indexOf('password')>=0) {
                var choice=yield* neosysconfirm(db.response.split('|').join('\n'),1,'Try a different password, database or user code','Reset Password')
            } else {
                var choice=yield* neosysinvalid(db.response.split('|').join('\n'))
            }
            if (choice == 2) {
                neosyssettimeout('gpasswordreset_button.click()',100)
            } else {
                gpassword_element.focus()
                gpassword_element.select()
            }
...
function* hidepassword(chainexpression) {
    if (gsavedpasswordelement) {
        showpassword_sync()
        neosyssettimeout(chainexpression,1)
        return true
    }
    return false
}
