...
function* dict_AUTHORISATION(parameters) {
    ...
    di.link = 'yield* windowopenkey("../neosys/users.htm")'
    //need to allow to move user up/down
    //di.unique=true
    di.noinsertrow = !(yield* neosyssecurity('USER CREATE'))
    //allow deleting blank rows if allowed to create users but how to stop them creating new groups or joining groups 
    di.nodeleterow = !(yield* neosyssecurity('USER DELETE')) && di.noinsertrow
    ...
    if (!(yield* neosyssecurity('AUTHORISATION UPDATE GROUPS'))) di.readonly = gmsg
    neosys_dict_text(di)
    di.length = 40
    di.lowercase = false
    ...
    if (!(yield* neosyssecurity('AUTHORISATION UPDATE LOCKS'))) {
        di.readonly = gmsg
        di.nodeleterow = true
        di.noinsertrow = true
    }
    ...
    if (!(yield* neosyssecurity('AUTHORISATION UPDATE LOCKS'))) di.readonly = gmsg
    //prevent accepting bad white space characters like 160 nbsp which dont act as key separators (on LOCKS and KEYS)
    di.validcharacters='ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_ '
