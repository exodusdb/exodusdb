...
//debug in php like this
	...
	$debugging=time()<=(strtotime("2020-02-19")+86400);
	//moved further down
...
// session
	...
	//$neosysrootpath = preg_replace('!/neosys.net/!','/',$neosysrootpath);
        //$neosysrootpath = str_replace('/neosys.net/','/',$neosysrootpath);
        //$neosysrootpath = str_replace('/exodus/','/',$neosysrootpath);
        $neosysrootpath = preg_replace('!/[^/]*/3/neosys/scripts!','/',__DIR__);
...
if ($database) {
	...
	else if ($gdatabase_codepage=='UTF-8')
		...
		while (is_file($linkfilename . '.1')){
			...
			if (time() >= $waituntil){
				...
				if ($gautostartdatabase) {
					...
					//no autostart
					$config=ReadAll("{$neosysrootpath}NEOSYS//NET.CFG");
					if (strpos($config,"AUTOSTART=YES") !== false) {
						...
						//D:\neosys\NEOSYS\~9337586.RUN
						$runrequestfilename = $neosysrootpath . "NEOSYS" . $gslash . $linkfilename0 . ".RUN";
...
function unescape($str) {
	...
	$str=preg_replace_callback('/%u([0-9a-fA-F]{4})/', function ($match) {return json_decode('"\u'.$match[1].'"');}, $str);

	//	%FF decoded to UTF-8
	$str=preg_replace_callback('/%([0-9a-fA-F]{2})/', function ($match) {return json_decode('"\u00'.$match[1].'"');}, $str);
