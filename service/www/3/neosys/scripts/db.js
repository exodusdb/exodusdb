...
function neosys_dict_url(di,protocol) {
 ...
 neosys_dict_text(di)
 di.validation='yield* neosys_val_url("'+protocol+'")'
}
...
function neosys_dict_ipno(di,many,wildcard,plus) {
...
function neosys_dict_emailaddress(di,sepchar) {
 ...
 di.validation='yield* neosys_val_emailaddress('+sepchar+')'
}
...
function* neosys_val_diskdrive() {
 ...
 //if (!gvalue.match(/^[ABCDEFGHIJKLMNOPQRSTUVWXYZ](:)?$/))
  //return yield* neosysinvalid('Please enter a letter A-Z')
...
function neosys_dict_diskdrive(di) {
 ...
 di.validation='yield* neosys_val_diskdrive()'
 di.length=1
 di.maxlength=1
}
...
function neosys_dict_index(di,filename,fieldname,many,allownew,warnnew) {
 ...
 if (!di.validation) di.validation='yield* form_val_index('+filename.neosysquote()+','+fieldname.neosysquote()+','+allownew+','+warnnew+')'

 if (!di.popup) di.popup='yield* form_pop_index('+filename.neosysquote()+','+fieldname.neosysquote()+','+many+')'
...
function neosys_dict_time(di,mode,otherid,params) {
 ...
 if (mode&&otherid) {
  ...
  di.defaultvalue='yield* neosys_def_time("'+mode+'","'+otherid+'")'
  di.validation='yield* neosys_val_time("'+mode+'","'+otherid+'")'
 }
...
function neosys_dict_period(di,mode,otherperiodid,conversion) {
 ...
 di.align='R'
 di.length=5
 if (di.type=='F') di.validation='yield* neosys_val_period("'+mode+'","'+otherperiodid+'")'
}
  ...
  with (this.dict[dictn]) {
   ...
   for (var propname in temp) {
    ...
    if (!propname.match(validpropnames))
        alert(neosysquote(propname)+' invalid dictionary property in '+name+' ignored')
   ...
   if(typeof functioncode=='string'&&functioncode) {
    try {
        ...
        if (e.description)
            msg+=e.description
        msg+='\n\n'+functioncode
        return systemerror('neosysrecord()',msg)
    }
   }
        ...
        if (typeof(fieldno)!='number') {
         ...
         if(typeof(temp)=='undefined'||temp==null) {
          ...
          if (temp.indexOf(String.fromCharCode(0))>=0) {
           ...
           systemerror('neosysrecord_rev2obj()','char(0) in data from server in '+name+' length:'+temp.length)
           return 0
          }
