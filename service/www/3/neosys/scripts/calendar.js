...
Calendar.prototype.create = function() {
	...
	//neosys to allow focus so keyevents get triggered
	...
	//neosys table.style.font 	 	= "small-caption";
	table.style.fontWeight 	= "normal";
	//neosys table.style.textAalign	= "center";
	//neosys
	...
	this._calDiv.onkeydown = function (e) {
		...
		//neosys if (kc < 37 || kc > 40) return true;
...
/*	
	...
	while (index < firstIndex) {
		...
		this._dateSlot[index].data.parentNode.style.border= "black";
		index++;
	}
*/
	var index = 0;
	while (index < firstIndex) {
	    var ptr=this._dateSlot[index]
		ptr.value = -1;
		ptr=ptr.data;//neosys
		ptr.data = String.fromCharCode(160);
		ptr=ptr.parentNode;//neosys
		ptr.className = "";
		ptr=ptr.style;//neosys
...
//window.status=new Date()+' '+today+' '+toISODate(d1)
        ...
        var ptr=this._dateSlot[index];//neosys        
		ptr.value = i;
		ptr.data.data = i;
		
		ptr=ptr.data.parentNode;//neosys
		ptr.className = "";
	...
	if(this._showing) {
		...
		if( this._bw.ie6 ) {
		    ...
		    // this._underDiv.removeNode(true);
             neosysremovenode(this._underDiv)
		}
	}
}
