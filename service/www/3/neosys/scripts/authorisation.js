...
function* authorisation_changeallemaildomains() {
    ...
    var reply = yield* neosysdecide('Change which domain?', olddomains)
    if (!reply)
        return yield* neosysinvalid()
    var olddomain = olddomains[reply - 1]
    var newdomain = ''
    while (true) {
        newdomain = yield* neosysinput('Change ' + olddomain + ' to what?', newdomain)
        if (!newdomain)
            return yield* neosysinvalid()
        if (!newdomain.match(/^[^\. -_][\w.-]*\.[a-z]{2,}$/)) {
            yield* neosysinvalid(newdomain + ' is not a valid email domain name.\rPut something like mycompany.com')
            continue;
        }
        break;
    }
...
function* user_showtasks(event) {
    ...
    var taskid = yield* neosysdecide('Tasks authorised for ' + userid, db.data, [[0, 'Task'], [1, 'Lock']], 0, '', '', inverted = true)
    if (!taskid)
        return false

    var recn = (yield* gds.getall('TASK_ID')).neosyslocate(taskid) - 1
    if (recn < 0)
        return false
...
function* form_prewrite() {
    ...
    if (!(yield* neosyssecurity('AUTHORISATION UPDATE GROUPS'))) {
        ...
        for (var rown = userrows.length - 1; rown >= 0; --rown) {
            ...
            if (userids[rown] == '' && userrows[rown].db_ordinal.text == '') {
                ...
                return yield* neosysinvalid('User in line ' + (rown + 1) + ' must not be left blank\r\r' + gmsg)
            }
        }
    }
    ...
    for (var usern=userids.length-1;usern>=0;--usern) {
        ...
        userkeys=userkeys.neosysconvert(';',' ').split(' ')
        for (var userkeyn=userkeys.length-1;userkeyn>=0;--userkeyn) {
            userkey=userkeys[userkeyn]
            var accumn1=accumkeys.neosyslocate(userkey)
            if (!accumn1) {
                ...
                var response=yield* neosysconfirm(question,1,'<u>R</u>emove','<u>L</u>eave','Cancel')
                if (!response){
                    focuson($$('KEYS')[usern])
                    return yield* neosysinvalid()
                }
    ...
    if (!newusers)
        return yield* neosysokcancel('OK to save?', 1)

    var reply = yield* neosysconfirm('OK to save?', 3, 'Email new users', 'Save only', 'Cancel')
    if (!reply)
        return false
...
function* form_postread() {
    ...
    gtasks_usern = (yield* gds.getall('USER_ID')).neosyslocate(gusername) - 1
    if (gtasks_usern < 0) gtasks_usern = 999999

    gtasks_updatehighergroups = yield* neosyssecurity('AUTHORISATION UPDATE HIGHER GROUPS')

    neosyssettimeout('yield* authorisation_postdisplay()', 10)
...
function* user_val_userid() {
    ...
    if (otherln1) {
        ...
        //if (otherln1-1<grecn&&!gtasks_updatehighergroups) return yield* neosysinvalid(msg)

        //check user wants to move the account here
        if ((yield* neosysyesno(msg + '\r\rDo you want to move it here?', 2)) != 1)
            return yield* neosysinvalid()
    ...
    if (gvalue != '---' && 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.indexOf(gvalue.slice(0, 1)) < 0)
        return yield* neosysinvalid('Username must start with an alphabetic letter A-Z')

    if (gusername.indexOf('NEOSYS') < 0 && gvalue.indexOf('NEOSYS') + 1)
        return yield* neosysinvalid('User name cannot include "NEOSYS"')

    //prevent hidden users
    if (gtasks_otheruserids.neosyslocate(gvalue))
        return yield* neosysinvalid(gvalue + ' user already exists in another group')
    ...
    function* user_haskeys() {
        ...
        for (var keyn = 0; keyn < keys.length; keyn++) {
            ...
            if ((key == 'NEOSYS' && gusername != 'NEOSYS') || gtasks_otherkeys.neosyslocate(key))
                return yield* neosysinvalid('You are not authorised to use key ' + key.neosysquote())
            keys[keyn] = key
        }
        gvalue = keys.join(' ')
        return true
    ...
    function* form_preinsertrow1() {
        ...
        if (!(yield* neosyssecurity('USER CREATE'))) return yield* neosysinvalid(gmsg)

        //prevent addition of users above yourself
        if (!gtasks_updatehighergroups && grecn <= gtasks_usern) return yield* neosysinvalid('You cannot insert users above yourself')
    ...
    function* form_predeleterow1() {
        ...
        if (!(yield* neosyssecurity('USER DELETE')))
        ...
        if (!gtasks_updatehighergroups && grecn <= gtasks_usern)
            return yield* neosysinvalid('You cannot delete yourself or higher users')

        //cannot delete group separator/blank lines or lines with keys
        if ((((yield* gds.get1('USER_ID')) == '') || (yield* gds.get1('KEYS'))) && !(yield* neosyssecurity('AUTHORISATION UPDATE GROUPS')))
            return yield* neosysinvalid('You cannot delete this line because\r' + gmsg)
    ...
    function* form_predeleterow2() {
        ...
        if ((yield* gds.get1('LOCKS')) && !(yield* neosyssecurity('AUTHORISATION UPDATE LOCKS')))
            return yield* neosysinvalid('You cannot delete this line because\r' + gmsg)
    ...
    function* task_authorised(task) {
        ...
        if (!(yield* neosyssecurity(task)))
            return yield* neosysinvalid('You cannot delete, change or add tasks that\ryou are not authorised to do yourself\r\r' + task)
    ...
    function* task_checkrank() {
        ...
        if (!gtasks_updatehighergroups && grecn < gtasks_usern) return yield* neosysinvalid('You cannot modify users above yourself')
