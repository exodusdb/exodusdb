...
Number.prototype.neosysdivide0isblank = function (divider) {
    ...
    if (typeof divider == 'string') {
        ...
        if (isNaN(divider2)) {
            return systemerror('neosysdivide0isblank()', 'divider "' + divider + '" is not an number')
            //return divider2
        }
        divider = divider2
    }
...
Array.prototype.neosyssplit = function neosyssplitarray(seps, invert) {
    ...
    for (var i = 0; i < this.length; i++) {
        ...
        if (typeof cell == 'undefined')
            systemerror('this['+i+']='+this[i]+' undefined','neosyssplit('+this+')')
        if (!cell.neosyssplit)
            systemerror('this['+i+']='+this[i]+' missing neosyssplit','neosyssplit('+this+')')
        newarray[i] = cell.neosyssplit(seps, invert)
    }
    return newarray
}

Array.prototype.neosysinvert = function neosysinvert(defaultvalue) {
...
Array.prototype.neosystrim = function neosystrimarray(trimchar) {
    var newarray = []
    var jj = -1
    for (var ii = 0; ii < this.length; ii++) {
        var cell = this[ii]
        if (cell) {
            cell=cell.neosystrim(trimchar)
            if (cell.length)
                newarray[++jj] = cell
        }
    }
...
function neosysassertfunctionname(arg1, funcname, varname) {
    ...
    return systemerror(funcname, varname + ' ' + neosysquote(arg1) + ' is not a function name')
}

function neosysassertstring(arg1, funcname, varname) {
    if (typeof (arg1) != 'string')
        return systemerror(funcname, varname + ' ' + neosysquote(arg1) + ' is not a string')
}

function neosysassertnumeric(arg1, funcname, varname) {
    if (!(neosysnum(arg1)))
        return systemerror(funcname, varname + ' ' + neosysquote(arg1) + ' is not numeric')
}
...
function NUMBER(mode, value, params) {
    ...
    if (mode == 'ICONV') {
        ...
        //minimum
        if (params[1] != '' && neosysnum(params[1]) && value < +params[1]) {
        ...
        if (params[2] && neosysnum(params[2]) && value > +params[2]) {
...
Array.prototype.neosysfield = function neosysfieldarray(sep, start, length) {
    ...
    var newarray = []
    for (var i = 0; i < this.length; i++)
        newarray[i] = this[i].toString().neosysfield(sep, start, length)
    return newarray
...
function neosys_HASHTAGS(type, input, mode) {

    //can handle an array of values but what about strings that appear as objects
    if (typeof input == 'object')
        return neosysconvarray(neosys_HASHTAGS, type, input, mode)
