...
$debugging = time() <= (strtotime("2020-04-24") + 86400);
//moved further down
...
//NEOSYS root path should point to the folder containing neosys, neosys.net, data, images etc
// eg D:\\hosts\\test or /var/www/exodus?
//$neosysrootpath = $_SERVER['DOCUMENT_ROOT'];
// __DIR__ = D:\\hosts\\test\\neosys.net\\3\\neosys\\scripts

//on linux
$neosysrootpath = preg_replace('!3/neosys/scripts!', '', __DIR__);
$neosysrootpath = preg_replace('!/neosys.net/!', '/', $neosysrootpath);

//on windows
$neosysrootpath = str_replace('3\\neosys\\scripts', '', $neosysrootpath);
$neosysrootpath = str_replace('\\neosys.net\\', '\\', $neosysrootpath);

if ($gwindows)
	$neosysrootpath = str_replace('/', '\\', $neosysrootpath);

//debug("neosysrootpath : $neosysrootpath");

// /var/www/html/neosys2//data/default.vol
// D:\hosts\test\neosys.net\3\neosys\scriptsNEOSYS/ADAGENCY.VOL
$gdatalocation = ($neosysrootpath . 'data' . $gslash);
...
debug("neosysdatapath :" . $gdatalocation);
...
if ($request0 == 'KEEPALIVE' || $request0 == 'LOOPBACK') {
	...
	if ($request0 == "EXECUTE") {
		$leadin = substr($data_io, 0, strlen($neosysrootpath)) . "/";
		debug("EXECUTE LEADIN " . $leadin);
		if (substr($data_io, 0, strlen($leadin)) == $neosysrootpath . "/") {
...
function getdatabases($neosysrootpath, $systemcode)
{
	...
	//$gdatalocation = $neosysrootpath . "/data/";
	global $gdatalocation;
	$volfilename = $gdatalocation . $systemcode . '.vol';
	//$volfilename=$neosysrootpath . "NEOSYS/" . $systemcode . '.VOL';
...
function writerec($file, $key, &$data_io)
{
	...
	for ($vn = 0; $vn < $maxvn; ++$vn) {
		...
		for ($fn = 0; $fn < $ncols; ++$fn) {
			...
			//debug([$cols[$fn], $cell],5);

			// (i)nt: replace '' with NULL
			if ($cols[$fn] == 'I') {
				//debug('I '.json_encode($cell),5);
				$dbcmd->bindValue($fn + 1, !strlen($cell) ? NULL : $cell, PDO::PARAM_INT);

				// (d)ate: replace '' with NULL
				// and convert gui intdate to sql date YYYY-MM-DD
			} elseif ($cols[$fn] == 'D') {
				//debug('D '.json_encode($cell),5);
				// maybe add || !$nullables[$fn]
				if (substr($colname, -8) == '_updated' || (!strlen($cell) && substr($colname, -8) == '_created')) {
					$dbcmd->bindValue($fn + 1, sqlnow(), PDO::PARAM_STR);
				} else {
					$dbcmd->bindValue($fn + 1, !strlen($cell) ? NULL : intdate2sql($cell), PDO::PARAM_STR);
				}
				// (f)loat replace '' with NULL
			} elseif ($cols[$fn] == 'F') {
				//debug('F '.json_encode($cell),5);
				$dbcmd->bindValue($fn + 1, !strlen($cell) ? NULL : $cell, PDO::PARAM_STR);

				// (j)son put []for null
			} elseif ($cols[$fn] == 'J') {
				//debug('J '.json_encode($cell),5);
				$dbcmd->bindValue($fn + 1, json_encode(explode(VM, $cell)), PDO::PARAM_STR);

				// (v)archar do not replace '' with NULL
			} else {
				//debug('? '.json_encode($cell),5);
				$dbcmd->bindValue($fn + 1, $cell, PDO::PARAM_STR);
			}
		}
		$result = $dbcmd->execute();
		if (!$result) {
			//debug($dbcmd->errorInfo(),5);
...
function get_cols(&$file)
{
	...
	for ($coln = 0; $coln < $result->columnCount(); $coln++) {
		...
		//debug(json_encode($col),5);
		/*
		debug($col['native_type'],5);
		VARCHAR "VAR_STRING"
		INT "LONG"
		INT "LONG"
		JSON null
		...
		*/
		...
		//		debug($col_type,5);
		if ($col_type == 'LONG' || strpos($col_type, 'INT') !== false)
			$col_type = 'INT';
