...
function* form_postinit() {
    //neosyssettimeout('focuson("NOTES")', 1000)
...
function* form_prewrite() {
    if ((yield* gds.getx('SCOPE'))=='SYSTEM.CFG' && !(yield* gds.getx('HOST_NAME')))
        return yield* neosysinvalid('Host Name is required when Scope is "Current Installation"')
    return true
}
...
function* sysconfig_copynow(event) {
    ...
    if (!(yield* neosysyesno(question,2)))
        return false
...
function* dict_SYSTEMCONFIGURATION(parameters) {
    ...
    neosys_dict_ipno(di, true, true, true)
    ...
    yield* general_dict_datasetcode(di, true, false)
    //di.required=true;
    di.unique = true
    ...
    yield* general_dict_datasetcode(di, true, false, true)
