...
function neosys_client_init() {
    ...
    document.writeln('<link id="neosys_global_css" rel="stylesheet" type="text/css" href="' + NEOSYSlocation + 'global.css">')
...
function neosys_set_style(mode, value, value2) {
    ...
    var ss = document.getElementById('neosys_global_css');
...
function* clientfunctions_getglobals() {
   gcompanycode = neosysgetcookie2('cc')
    ...
    gmaxstrsize = Number(neosysgetcookie2('ms'))
...
function add_neosys_menubar() {

    //if no neosys_menubar element
    //create a neosys_menu span
    // at the beginning of the body
    // or after the first navbar element

    gneosys_menubar=document.getElementById('neosys_menu')
    if (!gneosys_menubar) {
...
//    gdataset = neosysgetcookie2('dataset')
    ...
    //add menu, logout and refresh buttons if not a popup, depending on gshowmenu, not /neosys/ location and no navbar elements
    //if no neosys_menu span (even if no menu, it is a holder for NEOSYS form buttons New/Save etc.)
    if (!window.dialogArguments && (typeof gshowmenu == 'undefined' || gshowmenu) && NEOSYSlocation != './neosys/' && document.getElementsByClassName('navbar').length == 0) {
	...
	//insert buttons in reverse order
        ...
        if (typeof gshowrefreshcachebutton=='undefined'||gshowrefreshcachebutton) {
            ...
            gneosys_menubar.insertBefore(temp2, gneosys_menubar.firstChild)
        ...
        if (gmenucodes && gmenucodes != 'EXIT2') {
            ...
            gneosys_menubar.insertBefore(menu_span, gneosys_menubar.firstChild)
            ...
            //gneosys_menubar.insertBefore(dropdown_menu_span, gneosys_menubar.firstChild)
...
function neosysdblink() {
    ...
    gusername = neosysgetcookie2('username') || gusername
    //alert('xyz2 NEOSYS2/username='+neosysgetcookie2('username'))
    //alert(document.cookie)
    gsystem = neosysgetcookie2('', 'NEOSYSsystem',null) || gsystem
...
function cancel_backpage_event(event) {
    ...
    if (!document.getElementById('neosysconfirmdiv')) {
        history.back()
        return
    }
    
    //remove the popup and its controlling generator/coroutine
    //neosysremovenode(neosysconfirmdiv)
    //geventhandler = false
    neosys_resume(false,'cancel_backpage_event')
