...
nocommon:
	...
	if (not(file.open(filename, ""))) {
		response = "Error: " ^ (DQ ^ (filename ^ DQ)) ^ " file is not available";
	...
	if (linkfilename2) {
		...
		if (not(linkfile2.osopen(linkfilename2))) {
			response = "Error: " ^ (DQ ^ (linkfilename2 ^ DQ)) ^ " cannot open output file";
	...
	if (not(DICT.open("DICT", dictfilename))) {
		response = "Error: " ^ (DQ ^ ("DICT." ^ filename ^ DQ)) ^ " file is not available";
			...
			if (not(dictrec.read(DICT, dictid))) {
				if (not(dictrec.read(dictvoc, dictid))) {
					if (dictid == "ID") {
						...
						response = "Error: " ^ (DQ ^ (dictid ^ DQ)) ^ " IS MISSING FROM DICT." ^ filename;
		...
		if (not(LISTACTIVE or ((" " ^ sortselect).index(" BY ")))) {
			if (records.read(file, "%RECORDS%")) {
				if (records.length() < 200) {
					records.swapper(FM, "\" \"");
					sortselect.splicer(1, 0, DQ ^ (records ^ DQ) ^ " ");
				}
			}
		}
...
///////////
	...
	for (var limitfieldn = 1; limitfieldn <= nlimitfields; ++limitfieldn) {
		...
		if (limitcheck == "EQ") {
			...
			call mssg(DQ ^ (limitcheck ^ DQ) ^ " invalid limitcheck in select2");
	...
	if (dictids == "RECORD") {
		...
		for (var dictidn = 1; dictidn <= ndictids; ++dictidn) {
			...
			if (xml) {
				...
				row ^= "<" ^ dictid2 ^ ">" ^ cell ^ "</" ^ dictid2 ^ ">" ^ crlf2;
			}else{
		...
		if (xml) {
			row = "<RECORD>" ^ crlf2 ^ row ^ "</RECORD>" ^ crlf2;
			//move up
			//swap '&' with '&amp;' in row
			//swap "'" with "" in row
		}
