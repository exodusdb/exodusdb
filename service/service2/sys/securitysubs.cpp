...
//WARNING TODO: check trigraph following;
	...
	//called as prewrite in noninteractive mode
		...
		if (not interactive) {
			if (not(win.registerx(7).read(DEFINITIONS, "SECURITY"))) {
			...
			for (usern = 1; usern <= nusers; ++usern) {
				...
				if (not interactive) {
					...
					if (newpassword and (newpassword.length() < 4)) {
						msg = DQ ^ (userx ^ DQ) ^ " user password cannot be less than " ^ 4;
						return invalid(msg);
					}

					if (not(win.registerx(7).a(1).locateusing(userx, VM, ousern))) {
						ousern = 0;
					}
			...
			for (var taskn = 1; taskn <= ntasks; ++taskn) {
				...
				if (task) {
					if (not(win.orec.a(10).locateusing(task, VM, newtaskn))) {
						...
						if (not(RECORD.a(10).locateby(task, "AL", newtaskn))) {
	...
	*/
		...
		for (usern = 1; usern <= nusers; ++usern) {
			...
			if (not(userx.index("---"))) {
				...
				var menuid = "";
				if (win.registerx(7).a(1).locateusing(userx, VM, ousern)) {
	...
	//call oswrite(temprec,'x')
		...
		cmd ^= " HEADING " ^ (DQ ^ ("LIST OF " ^ mode2 ^ "   \'T\'   Page \'PL\'" ^ DQ));
...
//L6614:
	...
	569;
			...
			if newpassword = '' or newpassword = \1B\ then return;
