...
function main() {
	...
	live = datasetcode.substr(-4,4) ne "TEST";
	//if datasetcode='' then datasetcode='DEFAULT'

	neopath = "..\\NEOSYS\\";
	//install.end=neopath:'GLOBAL.END'
	installend = "GLOBAL.END";
	//server.end='..\..\':'GLOBAL.END'
	serverend = "..\\..\\" ^ SYSTEM.a(123) ^ ".END";
	databaseend = datasetcode ^ ".END";
	...
	printl(var().chr(196).str(80), "NEOSYS.NET SERVICE ", SYSTEM.a(24), " STARTED ", var().timedate());
	printl();
	...
	//subdirs=''
	//call subdirs('..\LOGS\':datasetcode:\00\,subdirs)
	//if count(subdirs,fm) else
	if (not(xx.osopen("..\\LOGS\\" ^ datasetcode))) {
		if (STATUS ne 2) {
			//call shell('MD ..\LOGS\':datasetcode)
			//call mkdir('..\LOGS\':datasetcode:char(0),xx)
			call osmkdir("..\\LOGS\\" ^ datasetcode);
		}
	}
	...
	if (logfile.osopen(logfilename)) {
		...
		//osbread rr from logfile at logptr-6 length 6
		t2 = logptr - 6;
		call osbread(tt, logfile,  t2, 6);
		if (tt == "</Log>") {
			logptr = t2;
		}
...
////////
	...
	//print 'lock locks,REQUEST*':linkfilename1
		...
		if (not(linkfile1.osopen(linkfilename1))) {
			...
			call ossleep(1000*.1);
			if (tracing) {
				printl("CANNOT OPEN ", DQ ^ (linkfilename1 ^ DQ));
			}
			goto nextlinkfile;
		}
...
subroutine requestinit() {
	...
	if (request1 == "CACHE") {
		USER0.eraser(firstrequestfieldn);
		request1 = USER0.a(firstrequestfieldn);
	}
	request1.ucaser();
	if (request1 == "LOGIN") {
	...
	if (logfilename) {
		...
		tt ^= " File=" ^ xmlquote(field2(replyfilename, "\\", -1));
		...
		logx.r(1, request1);
		if (logx.a(1)) {
			tt ^= " Req1=" ^ (DQ ^ (logx.a(1) ^ DQ));
		}
		if (logx.a(2)) {
			tt ^= " Req2=" ^ (DQ ^ (logx.a(2) ^ DQ));
		}
		if (logx.a(3)) {
			tt ^= " Req3=" ^ (DQ ^ (logx.a(3) ^ DQ));
		}
		if (logx.a(4)) {
			tt ^= " Req4=" ^ (DQ ^ (logx.a(4) ^ DQ));
		}
		if (logx.a(5)) {
			tt ^= " Req5=" ^ (DQ ^ (logx.a(5) ^ DQ));
		}
		req6up = logx.field(FM, 6, 9999);
		req6up.converter(FM, "^");
		if (req6up.length()) {
			tt ^= " Req6up=" ^ (DQ ^ (req6up ^ DQ));
		}
	...
	if (linkfile2size > maxstr) {
		...
		if (linkfile2.osopen(linkfilename2)) {
			...
			for (blockn = 1; blockn <= nblocks; ++blockn) {
				...
				call osbread(datx(blockn), linkfile2,  tt, inblocksize);
			...
			//check max iodat size <= maxstr
			if (lendata > maxstr) {
				USER1 = "";
				//response='Error: Maximum record size of 64Kb exceeded in LISTEN'
				call listen4(2, USER3);
				listenfailure = 1;
...
cannotopenlinkfile2:
		...
		if (not linkfile2size) {
			...
			call osbread(USER1, linkfile2,  0, maxstr);
	...
	if (logfilename) {
		logx = "\r\n" "<Response ProcessingSecs=" ^ (DQ ^ (secs ^ DQ)) ^ ">";
		gosub writelogx2();
		...
		if (linkfile2.osopen(linkfilename2)) {
			...
			for (blockn = 1; blockn <= nblocks; ++blockn) {
				...
				if (blk.length()) {
					...
					for (ii = 249; ii <= 255; ++ii) {
						blk.swapper(var().chr(ii), hexx(ii));
						//should not be done per block but is code economic
						USER3.swapper(var().chr(ii), hexx(ii));
					};//ii;
...
//call oswrite(username:fm:password:fm:encrypt0:fm:userprivs,date()[-3,3]:time():'.$$$')
	...
	//read a record
		...
		//if withlock then
		// if security(filename2:' UPDATE',updatenotallowed,'') then null
		// if security(filename2:' CREATE',createnotallowed,'') then null
		// end
...
getnextkey:
		...
		if (USER1.read(file, keyx)) {
			...
			if (withlock) {
				if (not(authorised(filename2 ^ " UPDATE"))) {
					goto noupdate;
				}
...
noupdate:
			...
			if (not FILEERROR or (FILEERROR.a(1) == "100")) {
				...
				if (withlock and not postread) {
					...
					//must provide a key unless locking
					if (not(authorised(filename2 ^ " CREATE", createnotallowed))) {
						if (sessionid) {
							gosub unlock();
						}
				...
				//response='Error: ':quote(keyx):' does not exist in the ':quote(newfilename):' file'
				USER1 = "";
...
badwrite:
			...
			if (RECORD == "") {
				//response='Cannot delete because ':quote(keyx):' does not exist in the ':quote(newfilename):' file'
				call listen4(15, USER3, keyx ^ FM ^ newfilename);
				gosub fmtresp();
				return;
			}
...
subroutine checkcompany() {
	...
	if (not(xx.read(allcols, filename ^ "*COMPANY_CODE"))) {
		return;
	}
	...
	if (authorised("COMPANY UPDATE " ^ (DQ ^ (compcode ^ DQ)), badcomp)) {
		return;
	}
