...
nextstatistic:
		if (readnext(ID)) {
			...
			hourn = (hourn - 1) % 24 + 1;
			for (ii = 1; ii <= 3; ++ii) {
				if (not(usertab(hourn, ii).locateusing(netid(ii), VM, tt))) {
...
nextprocess:
	if (readnext(ID)) {
		...
		if (not(dbasecodes.a(1).locateusing(dbasecode, VM, dbasen))) {
	...
	for (dbasen = 1; dbasen <= ndbases; ++dbasen) {
		...
		if ((((SYSTEM.a(17, 1).substr(-4,4) ne "TEST") or SYSTEM.a(126)) and (nok < minreq)) and (nhung < 5)) {
			...
			startit = not (dbasecode ^ ".END").osfile();
			if (startit) {
				//dirs=''
				//call subdirs('..\DATA\':dbasecode:'\*.*':\00\,dirs)
				//if dirs else startit=0
				//above doesnt work for unknown reason
				if (not(("..\\DATA\\" ^ dbasecode ^ "\\GENERAL\\REVMEDIA.LK").osfile())) {
					startit = 0;
				}
			}
			if (startit) {
	...
	//backupdrive='1'
			...
			if (not(paramrec.osread("..\\data\\" ^ dbasecode ^ "\\params2"))) {
				paramrec = "";
			}
			lastbackupsize = paramrec.a(3);
			if (lastbackupsize) {
				description ^= " " ^ lastbackupsize.oconv("[XBYTES,1]");
			}
	...
	//lastbackupsize=999999999
			...
			if (not(backupdrives.a(1).locateusing(backupdrive, VM, backupdriven))) {
				backupdrives.r(1, backupdriven, backupdrive);

				//ensure something is on the target
				//otherwise diskfreespace fails
				//call shell2('MD ':backupdrive:'\DATA.BAK')
				//call mkdir(backupdrive:'\DATA.BAK':char(0),xx)
				call osmkdir(backupdrive ^ "\\DATA.BAK");
				//pcperform 'md ':backupdrive:'\DATA.BAK'
				...
				if (tt ne testdata) {
					...
					nextbackupfilename = backupdrive ^ "\\DATA.BAK\\" ^ dbasecode ^ "\\" ^ dow ^ "\\BACKUP.ZIP";
...
nextdbasen:;
	...
	for (backupdriven = 1; backupdriven <= nbackupdrives; ++backupdriven) {
		...
		if (present) {
			...
			description = " Drive " ^ backupdrive ^ " Free:" ^ freespace.oconv("[XBYTES,1]");
			...
			if (lastbackupsize) {
				description ^= " Req:" ^ lastbackupsize.oconv("[XBYTES,1]");
			}
	...
	if (SYSTEM.a(124) and (SYSTEM.a(17).substr(-4,4) ne "TEST")) {
		...
		if (not upgradefiledir) {
			...
			upgradefilename83 = shell2("DIR " ^ (DQ ^ (longupgradefilename ^ DQ)) ^ " /x /l", errors);
			if (not errors) {
