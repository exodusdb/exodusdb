...
fail:
	...
	ok = 0;
	if (SECURITY.a(1).locateusing(userx, VM, usern)) {
		if (SECURITY.a(4, usern, 2).field(TM, 7) == "") {
			ok = 1;
			goto okfail;
		}
	...
	if (xdata == "?") {
		...
		if (not(userx == lockx.substr(1,userx.length()))) {
			...
			if (SECURITY.a(1).locateusing(userx, VM, usern)) {
				lockx ^= " " ^ SECURITY.a(4, usern, 2).field(TM, 7).oconv("HEX");
			}
		}
	...
	//inp.key:
		var keyx = "";
		call mssg("The lock is " ^ (DQ ^ (lockx ^ DQ)) ^ "|What is the key ?", "RC", keyx, "");
...
okfail:
	...
	if (not ok) {
		...
		if (SYSTEM.a(33)) {
			msg = "INVALID USERNAME OR PASSWORD " ^ (DQ ^ (userx ^ DQ));
