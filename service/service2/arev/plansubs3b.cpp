	...
	if (mode.substr(1, 8) == "SHOWTOTS" or (mode.field(",", 1)).index("GETFREE", 1)) {
		...
		for (var lnx = 1; lnx <= nlines; ++lnx) {
			...
			if (vehiclecode) {
				...
				if (number) {
					var vehicle;
					if (not(vehicle.reado(agy.vehicles, vehiclecode))) {
						vehicle = vehiclecode;
					}
					...
					// end
					if (not(array.a(1).locatebyusing(suppliername, "AL", vn, VM))) {
...
force error here TODO: check trigraph following;
					var wording = " " ^ extras ^ " ad" ? (if (array.a(4, lnx) == 1) {) : " " "s";
			...
			if (ANS.count(FM) > 1) {
				...
				if ((array.a(5)).sum()) {
					...
					var extras = "Unpaid";
force error here TODO: check trigraph following;
					var wording = " " ^ extras ^ " ad" ? (if ((array.a(4)).sum() == 1) {) : " " "s";
					wording = wording.oconv("L#15");
					wording ^= " = " ^ freevalue;
	...
	//Vehicle
force error here TODO: check trigraph following;
					tt = tt ? (if (agy.agp.a(2) == "CYP") {) : (lang.a(3) ^ lang.a(1));
	//Medium
					tt ^= VM ^ lang.a(127) ^ VM;
				}
	//Type
	...
	//Value
				...
				if (html) {
					tt ^= " (" ^ var("CURRENCY_CODE").calculate() ^ ")";
				}
	//% of paid
				if (anyperc) {
					tt ^= VM ^ lang.a(130);
		...
		for (var lnx = 1; lnx <= nlines; ++lnx) {
			...
			if (1) {
				...
				if (not msg.index(suppliername.field("|", 1)).substr(1, 13, 1)) {
