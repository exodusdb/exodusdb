...
function main() {
	...
	if (fin.definition.a(6).locateusing(vouchertype, VM, temp)) {
		...
		if (var("JO,OB,TR,RV").locateusing(internalvouchertype, ",", xx)) {
			...
			vtypereverse = var("D+").index(fin.definition.a(4, temp)).substr(1, 1, 1);
			if (not vtypereverse) {
...
subroutine postdirect() {
	...
	if (type == "SI" or type == "CN" or type == "PI" or type == "PC" or type == "INV" or type == "PIN") {
		daybookstyle = 2;
...
L760:
	...
	if (mode ne "DELETE" and voucher.a(12) == "") {
		...
		if (mode == "POST") {
			...
			if ((USER4.ucase()).index("POSTED OK", 1)) {
				//execute postcmd
		...
		if (fin.account.read(fin.accounts, origaccno)) {
			...
			if (not summaryacc) {
				...
				if (not deleting) {
					...
					var balances;
					if (not(balances.read(fin.balancesfile, balkey))) {
						balances = "";
					}
					if (not(balances.a(16).locatebyusing(currencycode, "AL", vn, VM))) {
						if (currencycode ne fin.basecurrency) {
							balances.inserter(16, vn, currencycode);
							//put a record there too
							var("").write(fin.balancesfile, balkey.fieldstore("*", 4, 1, currencycode));
						}
						balances.write(fin.balancesfile, balkey);
					}
				}
...
postindirect:
	...
	call osbwritex(line, outputfile, outputfilename, outputfilename.osfile().a(1));
	outputfile.osclose();
