...
function main() {
	//jbase
	//gbp common is copy of abp,common (updated by copyabp?)
	...
	if (gen.company.a(27)) {
		...
		SYSTEM.r(14, system.system.a(36));
	}
	...
	if (firstmonth.isnum()) {
		if (not(var("1" _VM_ "2" _VM_ "3" _VM_ "4" _VM_ "5" _VM_ "6" _VM_ "7" _VM_ "8" _VM_ "9" _VM_ "10" _VM_ "11" _VM_ "12").a(1).locateUsing(firstmonth, VM, temp))) {
			firstmonth = 1;
		}
	...
	if (DATEFORMAT.substr(1, 6) == "31/01/") {
		DATEFORMAT = "D2/E";
		goto 1040;
	}
	if (DATEFORMAT.substr(1, 6) == "31-01-") {
	...
	if (DATEFORMAT.substr(1, 6) == "01/31/") {
		DATEFORMAT = "D2/";
		goto 1040;
	}
	if (DATEFORMAT.substr(1, 6) == "01-31-") {
	...
	if (DATEFORMAT.substr(-6, 6) == "90/01/31") {
		DATEFORMAT = "D2J";
...
L1040:
	...
	for (var vn = 1; vn <= nvs; ++vn) {
		...
		if (temp) {
			...
			// LOCATE DEFINITION<1,VN> IN 'SIýCN'<1> SETTING TEMP THEN TEMP=0 ELSE TEMP=1
	...
	//number format (@USER2)
	USER2 = "MC";
	if (var("1,000.00" _VM_ "1000.00").a(1).locateUsing(gen.company.a(22), VM, temp)) {
		USER2 = "MD";
	}
	USER2 ^= ndec ^ "0P";
	if (var("1,000.00" _VM_ "1.000,00").a(1).locateUsing(gen.company.a(22), VM, temp)) {
		USER2 ^= ",";
	}
...
currencydescexit:
	...
	if (gen.company.a(2)) {
		...
		fin.gendesc = .convert(", "-", perioddesc);
		if (currencydesc) {
			fin.gendesc ^= ", " ^ .convert(", "-", currencydesc);
		}
		if (companydesc) {
			fin.gendesc ^= ",  " ^ .convert(", "-", companydesc);
		}
	}else{
		...
		if (s23.locateUsing("TESTDATA", VM, xx!!!)) {
			...
			if (s23.locateUsing("TESTING", VM, xx!!!)) {
				...
				if (s23.locateUsing("TRAINING", VM, xx!!!)) {
					...
					if ((system.system.a(17)).index("TEST", 1)) {
						USERNAME ^= "*";
					}
				}
			}
