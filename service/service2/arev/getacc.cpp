...
force error here TODO: check trigraph following;
	var positive = (not authorised(ledgerorbalances ^ " ACCESS", xx, "")) ? "#" : "";
...
vouchertype:
	...
	if (mode.substr(1, 13) == "VVOUCHER.TYPE") {
		doctype = "VOUCHER";
		mode.splicer(1, 1, "");
		goto 412;
	}
	if (mode.substr(1, 13) == "IVOUCHER.TYPE") {
		doctype = "INVOICE";
		mode.splicer(1, 1, "");
	}
L412:
...
nextbatch:
						...
						if (temp.reado(fin.batches, reqcompany ^ "*" ^ vtype ^ "*" ^ lastbatchno ^ suffix)) {
							lastbatchno += 1;
							goto nextbatch;
						}
					}else{
...
msgexit:
		...
		if (ANS == "") {
			...
			for (var replyn = 1; replyn <= nreplies; ++replyn) {
				...
				if (temp) {
					var temp2;
					if (not(temp2.reado(gen.companies, temp))) {
...
force error here TODO: check trigraph following;
			...
			for (var replyn = 1; replyn <= nreplies; ++replyn) {
				...
				if (fin.account.reado(fin.accounts, acno)) {
...
subroutine getacctypedesc() {
	if (reqledgertype == "STOCK") {
		accstk = "stock item";
		accstk2 = "stock code";
		goto 5273;
	}
	if (reqledgertype == "JOB") {
		accstk = "job account";
		accstk2 = "job number";
		goto 5273;
	}
