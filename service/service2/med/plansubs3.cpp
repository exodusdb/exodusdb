...
defversion:
		...
		xdict.transfer(DICT);
		if (not LISTACTIVE) {
			msg = DQ ^ (calculate("BRAND_CODE") ^ DQ) ^ " - BRAND HAS NO RECORDS IN THE MATERIAL FILE";
			return invalid(msg);
		}
		...
		if (RECORD.a(184).locateusing(win.is, VM, tt)) {
			msg = DQ ^ (win.is ^ DQ) ^ " material code is already used in line " ^ tt;
			return invalid(msg);
		}

		if (not(var("ABCDEFGHIJKLMNOPQRSTUVWXYZ").index(win.is))) {
		...
		materialdescs = trim(materialdescs, VM, "F");
		var material = "";
		...
		//skip coincidence check of media types with max minutes -1
		if (maxminutes and (maxminutes < 0)) {
			return 0;
		}
		...
		if ((maxminutes == "") and (var("TSR").index(reqmedium[1]))) {
		...
		if (maxminutes == 999) {
			//check media type and market per day
			maxminutes = "";
...
//L2050:
		...
		//buffer to save reading the vehicles file for every ad
		var wantedvehicles = "";
		var unwantedvehicles = "";
		...
		//1;All Clients:2;Client group:3;Client only:4;Brand only:5;None
		...
		if (clientcoincidencemode == 1) {
			...
			call btreeextract(tt, "CLIENTS", dictclients, reqclientcodes);
...
//L2299:
		//smdates for ii=1 to len(trimb(newdates))
		var nnewdates = newdates.count(SVM) + (newdates ne "");
		for (var ii = 1; ii <= nnewdates; ++ii) {
			//smdates if trim(newdates[ii,1]) and not(trim(olddates[ii,1])) then
			//if newdates<1,1,ii> and not(olddates<1,1,ii>) then
			if (newdates.a(1, 1, ii) ne olddates.a(1, 1, ii)) {
				...
				if (newdate >= 1) {
					...
					if (reqmedium) {
						...
						//tt:='VEHICLE_CODE':@vm:currvehiclecode:@fm
					}else{
						btreecode = "VEHICLE_AND_DATE" ^ VM ^ currvehiclecode ^ "*" ^ newdate ^ FM;
					}

					var hits = "";
					call btreeextract(btreecode, "ADS", dictads, hits);
					USER4 = "";
					if (hits) {
						...
						for (var hitn = 1; hitn <= nhits; ++hitn) {
							...
							//if same vehicle
							var advehiclecode = hit.field("*", 2);
							if (advehiclecode == currvehiclecode) {
...
coincident:
										...
										if (reqmedium) {
											coincidences ^= "<TD>" ^ advehiclecode ^ "</TD>";
										}
										coincidences ^= "</TR>";
								...
								//skip ad if not the same vehicle, and not filtering per media type
								goto 3251;
							}
							if (not reqmedium) {
								goto nextad;

								//if not the same vehicle, and filtering per media type
							}else{

								//check vehicles already found to be wanted (same media type and market)
								if (wantedvehicles.locateusing(advehiclecode, VM, xx)) {
									goto checkad;
								}

								//skip vehicles already found to be unwanted
								if (unwantedvehicles.locateusing(advehiclecode, VM, xx)) {
									goto nextad;
								}

								//first time this vehicle seen so get its details
								//missing vehicles go on the list in order not to be invisible
								if (not(vehicle.read(agy.vehicles, advehiclecode))) {
									goto checkad;
								}

								//check if the vehicle is wanted (same media type and market)
								if ((vehicle.a(2) == reqmedium) and (vehicle.a(4) == reqmarket)) {
									wantedvehicles ^= VM ^ advehiclecode;
									goto checkad;
								}

								unwantedvehicles ^= VM ^ advehiclecode;
...
//L3445:
			...
			if (interactive) {
				msg ^= "||" ^ var("Brand").oconv("L#20") ^ var(" Date").oconv("L#10") ^ (" " ^ tt).oconv("L#6") ^ var(" Schedule No").oconv("L#20");
			}
		...
		var tocurr = calculate("CURRENCY_CODE");
...
valexchrate:
		...
		var tocurr = agy.agp.a(2);
		goto valexchrate;

	//11/7/97
	//case mode='SHOWTOTS' or mode='GETFREE'
	} else if ((mode.substr(1,8) == "SHOWTOTS") or mode.field(",", 1).index("GETFREE")) {
		...
		call note("For which day or date do you want the programs ?|(You can omit the year and the month)", "RC", datex, "");
		if (not datex) {
			return 0;
		}
		datex = datex.field(" ", 1);
		...
		call note(var("Which channels do you want ?").oconv("L#50"), "RC", vehiclecodes, "");
		if (not vehiclecodes) {
			return 0;
		}
...
nextratings:
			...
			if (proglist) {
				...
				for (var progn = 1; progn <= nprogs; ++progn) {
					...
					if (rating) {
						if (not(rating.a(111).locateby(timex, "AR", ratingn))) {
							ratingn -= 1;
						}
		...
		var dayofweek = var("Monday,Tuesday,Wednesday,Thursday,Friday,Saturday,Sunday").field(",", daynofweek);
		...
		call note("If you want to specify the material/theme now,|What material will you use ?", "R", material, "");
		...
		for (var prognn = 1; prognn <= nprogns; ++prognn) {
			...
			if (RECORD.a(20, win.mvx) ne vehiclecode) {
				...
				win.is = vehiclecode;
				win.isorig = "";
				call plansubs2("VAL.vehiclecode");
				win.is = isx;
				win.isorig = isorigx;
...
subroutine getletter() {
	...
	for (var ii = 65; ii <= 90; ++ii) {
		if (lettersources.index(var().chr(ii))) {
