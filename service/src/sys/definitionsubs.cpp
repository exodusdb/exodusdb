...
function main(in mode) {
	...
	if (ID == "ALL") {
		call vouchertypesubs(mode);
		return 0;
	}

	if (ID == "TIMESHEET.PARAMS") {
		call timesheetparamsubs(mode);
		return 0;
	}
...
unlockdefinitions:
			...
			if ((win.wlocked and RECORD.a(8).index("NEOSYS")) and not(USERNAME.index("NEOSYS"))) {
preventupdate:
				win.wlocked = 0;
				xx = unlockrecord("DEFINITIONS", win.srcfile, ID);
			}
		...
		if (ID == "AGENCY.PARAMS") {
			...
			if (not(authorised("AGENCY CONFIGURATION UPDATE", xx, "NEOSYS"))) {
				goto preventupdate;
			}
		...
		if (((ID.field("*", 2) == "ANALDESIGN") and (USERNAME == "NEOSYS")) and RECORD.a(10)) {
			var reports;
