...
nextreport:
	...
	*/
	...
	//if 1 then
		...
		msg ^= "||NEOSYS needs at least " ^ reqfreemb ^ "Mb PER USER|of free space on disk " ^ oscwd().substr(1,2) ^ " but";
	...
	if (VOLUMES) {
		...
		var ext = "NET";
		var path = "../neosys." ^ ext ^ "/neosys/dll/";
	...
	if (xx.open("ACCOUNTS", "")) {
		if (xx.open("ABP", "")) {
			...
			if (msg) {
				...
				msg.swapper(FM, " ");
				if (interactive) {
					call note(msg.quote() ^ "|account missing - see company setup");
				}
			}
		}
	}
...
nextdoc:
	...
	if (version.osread("GENERAL" OSSLASH "VERSION.DAT")) {
