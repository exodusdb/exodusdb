	...
	if (mode.index("POSTREAD")) {
		...
		if (var(1) or win.wlocked) {
			...
			if (not(RECORD.read(versionfile, ID))) {
				...
				msg = ID.quote() ^ " missing from " ^ versionfilename ^ " file";
				return invalid(msg);
			}
		...
		//orec means updating
			...
			anythingchanged = 0;
			if (statusfn > 0) {
				//temporec=replace(orec,status.fn,0,0,@record<status.fn>)
				var temporec = win.orec;
				temporec.r(statusfn, RECORD.a(statusfn));
		...
		if ((win.datafile == "JOBS") or (win.datafile == "SCHEDULES")) {
			...
			if (indexvalues.read(DEFINITIONS, indexvalueskey)) {
				if (indexvalues.a(1).locateby("AL",execcode,execn)) {
...
updindexvalues:
		...
		if (module and request) {
			...
			if (not(SECURITY.a(10).locate(task,taskn))) {
				return 0;
			}
...
subroutine getfiledefaults() {
	...
	//only POSTREAD AND PREDELETE done here
		...
		msg = win.datafile.quote() ^ " unknown file in GENERAL.SUBS2," ^ mode;
		gosub invalid(msg);
		return;
	}
...
subroutine addlog() {
	...
	#define trimsize 0
	if ((RECORD.length() > trimsize) and (RECORD.a(userlogfn, trimsize, trimsize).count(VM) >= 20)) {
		if (logn == 1) {
			ii = 1;
		}else{
			ii = logn - 9;
