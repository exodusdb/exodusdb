...
function request_init() {
	...
	if (tt == "VERSION 3") {
		...
		tt = request_.f(2) - 1;
		request_.remover(2);
	} else if (request_.f(1) == "VERSION 2") {
		tt = 4;
	} else {
		tt = 0;
	}
	...
	if (request2 == "JOURNALS") {
		request2 = "JOURNALS";
		if (request3.count("*") == 3 and (request3.ends("*"))) {
			request3.popper();
		}
	}
...
subroutine process() {
	...
	// With try/catch so errors can be dealt with properly
		...
		try {
			...
			response_ = e.description.default_from("No error message") ^ FM ^ e.stack();
		}
...
subroutine process2() {
	...
	if (invaliduser) {
		...
		autokey = 0;
		if (((keyx == "" or (keyx.starts("*"))) or (keyx.ends("*"))) or keyx.contains("**")) {
...
getnextkey:
		...
		if (withlock) {
			...
			if (response_ == "NOT OK" and autokey) {
				goto getnextkey;
			}
...
noupdate:
		...
		if (postread) {
			...
			if (req.reset >= 5 or ((msg_ and (req.reset != - 1)))) {
				if (withlock) {
					gosub leaseunlock();
					// wlocked=0
				}
				...
				if (not(req.wlocked) and data_) {
					...
					if (keyx != keyx0 and not(response_.contains("RECORDKEY"))) {
						tt = keyx;
						tt.replacer(" ", "%20");
		...
		req.wlocked	  = sessionid;
		req.saverec	  = request1   != "DELETE";
		req.deleterec = request1 == "DELETE";
		...
		if (request1 != "DELETE") {
			...
			if (allcols != "") {
				if (dictrec.readc(allcols, filename ^ "*DATE_TIME")) {
					...
					if (olddatetime and olddatetime != newdatetime) {
						gosub properunlock();
...
function request_exit() {
	...
	if (response_ == "OK") {
		if (request1 == "STOPDB" or request1.starts("RESTART")) {
			// gosub main_exit();
			return false;
		}
	}
