...
function main() {
	...
	if (mode.field(".", 1, 3) == "DEF.ORDER.NO") {
	...
	if (mode == "VAL.ORDER.NO") {
		...
		if (RECORD.read(win.srcfile, win.is)) {
			...
			if (not(validcode2(var("COMPANY_CODE").calculate(), var("CLIENT_CODE").calculate(), var("BRAND_CODE").calculate(), agy.brands, msg))) {
				goto EOF_442;
			}
			if (not(validcode3(var("MARKET_CODE").calculate(), var("SUPPLIER_CODE").calculate(), "", "", msg))) {
				goto EOF_442;
			}
			RECORD = "";
...
lockit:
	...
	if (mode == "VAL.JOB") {
		...
		//get the job
		var job;
		if (not(job.reado(agy.jobs, win.is))) {
	...
	*/
			...
			if (supplier.a(9)) {
				if (fin.accounts.open("ACCOUNTS", "")) {
					if (fin.account.read(fin.accounts, supplier.a(9, 1, 2))) {
						if (fin.account.a(4)) {
							if (var("CURR_CODE").calculate() ne fin.account.a(4)) {
								if (not(decide2("WARNING:|This purchase order currency is " ^ (DQ ^ (var("CURR_CODE").calculate() ^ DQ)) ^ "|but the suppliers account is currency " ^ (DQ ^ (fin.account.a(4) ^ DQ)), "OK" _VM_ "Cancel", reply, 2))) {
									reply = 2;
								}
								if (reply ne 1) {
									win.valid = 0;
	...
	if (mode == "VAL.SUPPINV") {
		...
		for (var hitn = 1; hitn <= nhits; ++hitn) {
			...
			if (prodorderid ne ID) {
				...
				if (prodorder.read(win.srcfile, prodorderid)) {
					if (prodorder.a(10) == var("SUPPLIER_CODE").calculate()) {
						nsupplierhits += 1;
						...
						//if interactive then
						// if decide2('!WARNING|':tempmsg,'OKýCancel',reply,2) else reply=2
						//end else
		...
		if (nsupplierhits) {
			...
			tempmsg.r(-1, var("SUPPLIER_NAME").calculate());
	...
	if (mode == "VAL.AMOUNT") {
		win.is = win.is.oconv("MD" ^ var("NDECS").calculate() ^ "0P");

		goto 4641;
	}
	if (mode == "DEF.EXCH.RATE") {
}

subroutine defexchrate() {
		ANS = "";
		var currcode = var("CURR_CODE").calculate();
		if (not currcode) {
			return;
		}
		...
		var idate = var("INV_DATE").calculate();
		if (idate == "") {
			idate = var("DATE").calculate();
		}
		if (not(gen.currency.a(4).locatebyusing(idate, "DR", vn, VM))) {
			{}
		}
	...
	if (mode == "PREWRITE") {
		...
		if (tt.a(1, 1) == "") {
			...
			if (tt.a(1, 1)) {
				...
				tt = var("JOB_PRODUCTION_TYPE").calculate();
				if (tt) {
					var prodtype;
					if (not(prodtype.reado(agy.mediatypes, tt))) {
						prodtype = "";
					}
					if (not prodtype.a(5)) {
						goto badtype;
...
badtype:
		...
		if (RECORD.a(22) or taxcode.length()) {
			...
			if (not(fin.taxes.a(2).locateusing(taxcode, VM, taxn))) {
				{}
			}
	...
	if (mode.substr(1, 8) == "POSTREAD") {
		...
		if (RECORD.a(2)) {
			if (not(validcode2(var("COMPANY_CODE").calculate(), "", var("BRAND_CODE").calculate(), agy.brands, msg))) {
				win.reset = 5;
				goto EOF_442;
			}
			if (not(validcode3(var("MARKET_CODE").calculate(), var("SUPPLIER_CODE").calculate(), "", "", msg))) {
		...
		if (win.wlocked and var("JOB_CLOSED").calculate() == "Y") {
			if (RECORD) {
		...
		if (RECORD and RECORD.a(14) == "") {
				@record<14>={NDECS}
			RECORD.r(14, var("NDECS").calculate());
				orec<14>=@record<14>;
	...
	*/
		...
		if (win.orec and RECORD.a(24) == "") {
			...
			if (tt == "EXECUTIVE") {
				tt = var("EXECUTIVE_CODE").calculate();
			}
			RECORD.r(24, tt);
		}
...
initrec:
		...
		if (var("JOB_CLOSED").calculate()) {
			//if security('PRODUCTION ORDER CREATE AFTER JOB CLOSED',msg,'') else
			call note(DQ ^ (var("JOB_NO").calculate() ^ DQ) ^ " Job has been closed.");
			RECORD.r(2, "");
		}

		goto 4641;
	}
	if (mode == "POSTWRITE" or mode == "POSTDELETE") {
...
subroutine validate() {
	...
	var analysisfnx = 29;
	var brandcode = var("BRAND_CODE").calculate();
	var suppliercode = var("SUPPLIER_CODE").calculate();
	var marketcode = var("MARKET_CODE").calculate();

	var jobno = var("JOB_NO").calculate();
...
errexit:
	...
	//if @record<15>='' then
	if (trim2(var("OLD_PRODUCTION_TYPE").calculate(), VM) == "") {
	...
	if (var("DRAFT" _VM_ "CANCELLED").a(1).locateusing(RECORD.a(11), VM, xx)) {
		return;
	}
	...
	if (not deleting and (isinvoiced or var("SUPP_INV_NO").calculate())) {
	...
	fin.currfmt = "MD" ^ var("NDECS").calculate() ^ "0P";
	...
	// END
	MV = 0;
	var prodtypecodes = var("OLD_PRODUCTION_TYPE").calculate();
	var amounts = var("AMOUNT").calculate();
	var amountbases = var("AMOUNT_BASE").calculate();
	...
	if (var("SUPP_INV_NO").calculate()) {
		taxperc = var("TAX_PERC").calculate();
	}else{
		taxperc = "";
	}
	if (taxperc.length()) {
	...
	var firstprodtypecode = trim2(var("OLD_PRODUCTION_TYPE").calculate(), VM, "F").a(1, 1);
	//multitype
	var firstprodtype;
	...
	var accno = var("CLIENT_ACCNO").calculate();
	...
	if (var("SUPP_INV_NO").calculate() == "") {
		...
		var orderperiod = ((var("DATE").calculate()).oconv("D2/E")).substr(4, 5);
	...
	///////////////////////////////////////////////////////////////
	if (var("SUPP_INV_NO").calculate() == "" and not isinvoiced) {
		return;
	}
	...
	if (not accountingdate) {
		accountingdate = var("INV_DATE").calculate();
	}
	if (not accountingdate) {
		accountingdate = var("DATE").calculate();
	}
	...
	if (isinvoiced or var("SUPP_INV_NO").calculate()) {
	...
	if (agy.agp.a(102)) {
		if (var("SUPP_INV_NO").calculate()) {
			if (not var("SUPP_ACNO").calculate()) {
				msg = "You must setup the supplier A/c No. on the Supplier File|before entering supplier invoices for";
				msg ^= FM ^ FM ^ (DQ ^ (var("SUPPLIER_NAME").calculate() ^ DQ)) ^ " (" ^ var("SUPPLIER_CODE").calculate() ^ ")" ^ FM;
...
subroutine update() {
	...
	if (var("DRAFT" _VM_ "CANCELLED").a(1).locateusing(var("STATUS").calculate(), VM, xx)) {
		return;
	}
...
updaccounts:
	...
	if (1) {
		...
		if (var("SUPP_INV_NO").calculate()) {
			reference = "INV" ^ var("SUPP_INV_NO").calculate();
		}else{
			reference = "ORD" ^ ID;
		}
		voucher.r(13, reference);

		currencycode = var("CURR_CODE").calculate();
		...
		details = "~ Job:" ^ jobno ^ ", Order:" ^ ID ^ ", " ^ var("DESC").calculate();
		if (deleting) {
...
creditcostspertype:
				...
				accno = getagp(agy.agp.a(43), var("PERIOD").calculate());
			...
			if (isinvoiced) {
				...
				accno = getagp(agy.agp.a(14), var("PERIOD").calculate());
				accno.swapper("%SUPPLIER%", suppliercode);
				if (accno == "") {
					accno = jobno;
					if (jobno.isnum()) {
	...
	var invoice = brandcode ^ "*" ^ var("PERIOD").calculate();
	...
	if (isinvoiced) {
		...
		invoice.r(8, var("CURR_CODE").calculate());
		invoice.r(4, invamountbase);
	}else{
		invoice.r(12, invamount);
		invoice.r(13, var("CURR_CODE").calculate());
		invoice.r(14, invamountbase);
	}

	invoice.r(9, var("DESC").calculate());
	invoice.r(5, var().date());
	invoice.r(11, jobno);
	invoice.r(18, var("TAX_PERC").calculate());
...
updatenoinvoice:
	...
	if (job.a(6).locateusing(ID, VM, vn)) {
		if (var("SUPP_INV_NO").calculate() or deleting) {
			job.eraser(6, vn);
		}
	}else{
		if (not(var("SUPP_INV_NO").calculate() and not deleting)) {
			job.r(6, -1, ID);
		}
	}
...
subroutine checkjobunlocked2() {
	...
	if (not(job.reado(agy.jobs, jobcode))) {
...
subroutine addline() {
	...
	if (analysiscodex) {
		...
		//8 executive code
		analysiscodex = analysiscodex.fieldstore("*", 8, 1, var("EXECUTIVE_CODE").calculate().ucase());
	}
