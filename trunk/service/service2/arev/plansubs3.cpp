	...
	if (mode == "PERPETUAL") {
		...
		if (win.amvaction == 2) {
			...
			if (win.datafile == "SCHEDULES") {
				...
				//require credit note ?
				MV = win.mvx;
				if ((var("BILL").calculate() or var("COST").calculate()) and RECORD.a(46, win.mvx) ne "") {
...
L721:
	...
	if (mode == "VAL.DISCOUNT" or mode == "VAL.TAX") {
...
defversion:
	...
	if (mode == "F2.MATERIAL") {
		...
		var xm = MV;
		var xdict = DICT;
		perform("SSELECT MATERIALS WITH BRAND_CODE " ^ (DQ ^ (var("BRAND_CODE").calculate() ^ DQ)) ^ " \"\" (S)");
		xr.transfer(RECORD);
		xid.transfer(ID);
		xm.transfer(MV);
		xdict.transfer(DICT);
		if (not LISTACTIVE) {
			msg = DQ ^ (var("BRAND_CODE").calculate() ^ DQ) ^ " - BRAND HAS NO RECORDS IN THE MATERIAL FILE";
			goto EOF_455;
		}
	...
	if (mode == "VIEWMATERIAL") {
		...
		call materialsubs("VIEWMATERIAL-" ^ materialno ^ "-" ^ var("BRAND_CODE").calculate());
	...
	if (mode == "VAL.MATERIAL") {
		...
		call materialsubs("GETMATERIALCHAIN-" ^ win.is ^ "-" ^ var("BRAND_CODE").calculate());
		if (not win.valid) {
			return 0;
		}
		...
		materialdescs = trim2(materialdescs, VM, "F");
		var material = "";
	...
	if (mode == "SWITCHCOINCIDENCECHECKING") {
	...
	if (mode == "CHECKCOINCIDENCE") {
		...
		var newspec1 = (newspec.field(",", 1)).substr(1, "*");
		...
		var vehicle;
		if (not(vehicle.reado(agy.vehicles, currvehiclecode))) {
			vehicle = "";
		}
		...
		if (maxminutes == "") {
			//locate reqmedium in 'TVýTýSATýSýRADýR'<1> setting x then
...
checkad:
								...
								if (maxminutes == 0) {
									...
									if (newspec1 ne (ad.a(35).field(",", 1)).substr(1, "*")) {
										coincide = 0;
									}
...
force error here TODO: check trigraph following;
	...
	if (mode == "VAL.EXCH.RATE") {
		...
		var tocurr = var("CURRENCY_CODE").calculate();
...
valexchrate:
	...
	if (mode == "FINDPROG") {
		...
		//swap '/ ' with '/' in DATEX
		if (datex.field("/", 2, 2) == var("PERIOD").calculate()) {
			dom = (idate.oconv("D")).substr(1, 2) + 0;
		}else{
			for (dom = 1; dom <= 7; ++dom) {
			///BREAK;
			if ((dom ^ "/" ^ var("PERIOD").calculate()).iconv("D2/E") - 1 % 7 + 1 == daynofweek) break;;
			};//dom;
		}
		...
		for (var vehiclen = 1; vehiclen <= nvehicles; ++vehiclen) {
			...
			var ratingkey = var("SURVEY_CODE1").calculate() ^ "*" ^ var("TARGET1").calculate() ^ "*" ^ vehiclecode ^ "*" ^ dow;
			//try to get the diary ratings ...
			var rating;
			if (not(rating.read(agy.ratings, ratingkey))) {
nextratings:
				//otherwise try to get the people meter data
				ratingkey = var("SURVEY_CODE1").calculate().field(" ", 1) ^ "*" ^ var("TARGET1").calculate() ^ "*" ^ vehiclecode ^ "*" ^ idatex;
				if (not(rating.read(agy.ratings, ratingkey))) {
					rating = "";
					if (idatex > idate - 90) {
						idatex -= 7;
