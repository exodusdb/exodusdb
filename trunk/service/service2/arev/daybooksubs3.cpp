...
declare:
	...
	if (var("DEF.BATCH.REF" _VM_ "F2.BATCH.REF" _VM_ "VAL.BATCH.REF").a(1).locateusing(mode, VM, vn)) {
		win.valid = 1;
		ON vn GOTO defbatchref, f2batchref, valbatchref;
	}
...
valbatchref:
	...
	if (win.is ne win.registerx[4] and win.is ne win.isorig) {
		...
		if (not(tt.a(25).locateusing(USERNAME, VM, temp))) {
			if (not(authorised("JOURNAL ACCESS OTHER PEOPLES BATCHES", msg, ""))) {
				win.is = "";
				goto invalid2;
			}
		}
...
raiseunpostedminimum:
	...
	if (min.read(fin.batches, seqkey ^ "*")) {
		...
		if (min == ID.field("*", 3)) {
			...
			max = min;
L1528:   *go from minimum+1 to maximum but stopping at any existing batch;
	...
	//$insert abp,daybook.subs5
	//y2k
		//included in all previous daybook subroutines

	////////////
findvoucher:
	////////////
		//given LN1 somewhere in the voucher
		//return TOT.AMTS, TOT.BASES, TOT.CURRS
		// LN1 start and lines

		//find first line of the voucher
		while (true) {
			temp = RECORD.a(2, ln1);
		///BREAK;
		if (not(temp == DQ or temp == "")) break;;
			ln1 -= 1;
			if (ln1 < 1) {
				msg = "THE FIRST LINE OF THE BATCH MUST NOT BE A CONTINUATION LINE";
				var totamts = "";
				var totbases = "";
				var totcurrs = "";
				win.amvaction = "";
		...
		var totamts = RECORD.a(34, ln1);
		var totcurrs = RECORD.a(35, ln1);
		var totbases = RECORD.a(36, ln1);
...
addamts:
	////////
		...
		if (not(addamt.isnum() and addbase.isnum())) {
			return;
		}
		...
		if (add < 0) {
			//ADD.AMT=(-1*ADD.AMT) 'MD40P'
			//ADD.BASE=(-1*ADD.BASE) BASEFMT
			addamt.splicer(1, 0, "-");
			addbase.splicer(1, 0, "-");
			if (addamt.substr(1, 2) == "--") {
				addamt.splicer(1, 2, "");
			}
			if (addbase.substr(1, 2) == "--") {
				addbase.splicer(1, 2, "");
			}
		}

		if (totcurrs.a(1, 1).locateusing(addcurr, SVM, currn)) {
			totamts.r(1, 1, currn, 0 + ((totamts.a(1, 1, currn) + addamt).oconv("MD40P")));
			totbases.r(1, 1, currn, 0 + ((totbases.a(1, 1, currn) + addbase).oconv(fin.basefmt)));
			if (not(totamts.a(1, 1, currn) or totbases.a(1, 1, currn))) {
				totamts.eraser(1, 1, currn);
				totbases.eraser(1, 1, currn);
				totcurrs.eraser(1, 1, currn);
			}
		}else{
			if (addamt or addbase) {
				totamts.r(1, 1, currn, addamt);
				totbases.r(1, 1, currn, addbase);
				totcurrs.r(1, 1, currn, addcurr);
			}
		}
		return;
...
chksign:
	/////////
		//is journal if main account not in heading or lines
		var journal = not win.registerx[1].a(7);
		if ((win.registerx[7]).substr(1, 2) == "CR" or (win.registerx[7]).substr(1, 1) == "-") {
			var othersign = -1;
			var xsign = 1;
			goto 2123;
		}
		if ((win.registerx[7]).substr(1, 2) == "DR" or (win.registerx[7]).substr(1, 1) == "+") {
			var othersign = 1;
			var xsign = -1;
			goto 2123;
		}
		if (win.registerx[7] == "" or 1) {
			var othersign = 1;
			var xsign = 1;
		}
		return;
