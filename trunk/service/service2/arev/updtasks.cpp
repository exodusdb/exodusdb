...
function main() {
	...
	if (task.a(3) ne oldtask.a(3)) {
		...
		var clientname = calculatex("CLIENT_NAME", dictjobs, jobno, job, 0);
		body ^= " " ^ clientname;

		subject = body.a(1);

		body.r(-1, calculatex("FULL_BRAND_NAME", dictjobs, jobno, job, 0));
		body ^= ", " ^ calculatex("DESCRIPTION1", dictjobs, jobno, job, 0);
