...
function main() {
	...
	if (mode == "DELETEMATERIAL") {
		...
		if (not(dirlist2(imagelocation ^ materialcode))) {
		...
		if (dirlist2(imagelocation ^ materialcode)) {
	...
	if (mode.field("-", 1) == "VIEWMATERIAL") {
		...
		var imagefilenames = dirlist2(imagefilename ^ ".*");
	...
	if (mode == "GETSCHEDULEMATERIALS") {
		...
		//smdates
		//convert ' ý1234567890' to '' in dategrid
	...
	if (mode.field("-", 1) == "GETMATERIALCHAIN") {
	...
	if (mode == "POSTDELETE" or mode == "POSTWRITE") {
