...
function main() {
	...
	printl("\n========= test_precision ==========");
	{
		...
		//           999999999999999.875
		assert((var("999999999999999.9")    + 0).toString() == "9.999999999999999e14");
		printl( var("9999999999999999.9")   + 0);
		assert((var("9999999999999999.9")   + 0).toString() == "1e16");

#elif defined(USE_RYU)
		printl( var(1234567890123456789.)  ^ "x");
		assert((var(1234567890123456789.)  ^ "x") == "1.2345678901234568E18x");
		printl( var(12345678901234567890.) ^ "x");
		assert((var(12345678901234567890.) ^ "x") == "1.2345678901234567E19x");
...
#else
                                                    //1.234567890123457e+18x
                                                    //1.234567890123457e+19x
		//assert((var(1234567890123456789.)  ^ "x") == "1.2345678901234568E18x");
		//assert((var(12345678901234567890.) ^ "x") == "1.2345678901234567E19x");
		assert((var(1234567890123456789.)  ^ "x") == "1.234567890123457e+18x");
		assert((var(12345678901234567890.) ^ "x") == "1.234567890123457e+19x");

		printl( var("999999999999999.9")    + 0);
		//           999999999999999.875
		assert((var("999999999999999.9")    + 0).toString() == "9.999999999999999e+14");
		printl( var("9999999999999999.9")   + 0);
		assert((var("9999999999999999.9")   + 0).toString() == "1.000000000000000e+16");

#endif

#if USE_RYU
		//ryu conversion of ASCII numbers to doubles REJECTS too precise numbers that cannot be round tripped
		TRACE(var("99999999999999999.9"))
		// why are these ok now?
		//assert( ! var("99999999999999999.9").isnum());
		assert(var("99999999999999999.9").isnum());
		TRACE(var("0.999999999999999999"))
		...
		assert((var("99999999999999999.9")  + 0).toString() == "1E17");
		printl( var("999999999999999999.9") + 0);
		assert((var("999999999999999999.9") + 0).toString() == "1E18");
#endif
		TRACE(var(10)/var(3));
		...
		assert((var(999999999999999.9)    ^ "x") == "9.999999999999999E14x");
		assert((var(9999999999999999.9)   ^ "x") == "1E16x");
		assert((var(99999999999999999.9)  ^ "x") == "1E17x");
		assert((var(999999999999999999.9) ^ "x") == "1E18x");
		...
		*/
		...
		sv2="1.2345678901234568E28x";//ryu
#else
		...
		assert(test2("00000000000000000000.00000000000000000001",                   "1E-20x"));
#endif
		...
		assert(test2("00000000000000000000.00000000000000000012",                   "1.2E-19x"));
		assert(test2("00000000000000000000.00000000000000000123",                   "1.23E-18x"));
		assert(test2("00000000000000000000.00000000000000001234",                   "1.234E-17x"));
		assert(test2("00000000000000000000.00000000000000012345",                   "1.2345E-16x"));
		assert(test2("00000000000000000000.00000000000000123456",                   "1.23456E-15x"));
		assert(test2("00000000000000000000.00000000000001234567",                   "1.234567E-14x"));
		assert(test2("00000000000000000000.00000000000012345678",                   "1.2345678E-13x"));
		assert(test2("00000000000000000000.00000000000123456789",                   "1.23456789E-12x"));
		assert(test2("00000000000000000000.00000000001234567890",                   "1.23456789E-11x"));
		assert(test2("00000000000000000000.00000000012345678901",                   "1.2345678901E-10x"));
		assert(test2("00000000000000000000.00000000123456789012",                   "1.23456789012E-9x"));
		assert(test2("00000000000000000000.00000001234567890123",                   "1.234567890123E-8x"));
		assert(test2("00000000000000000000.00000012345678901234",                   "1.2345678901234E-7x"));
		...
		assert(test2("00000000000000000000.0000000000000000012345678901234567",     "1.2345678901234566E-18x"));
		assert(test2("00000000000000000000.000000000000000012345678901234567",      "1.2345678901234567E-17x"));
		assert(test2("00000000000000000000.00000000000000012345678901234567",       "1.2345678901234568E-16x"));
		assert(test2("00000000000000000000.0000000000000012345678901234567",        "1.2345678901234568E-15x"));
		assert(test2("00000000000000000000.000000000000012345678901234567",         "1.2345678901234567E-14x"));
		assert(test2("00000000000000000000.00000000000012345678901234567",          "1.2345678901234566E-13x"));
		assert(test2("00000000000000000000.0000000000012345678901234567",           "1.2345678901234567E-12x"));
		assert(test2("00000000000000000000.000000000012345678901234567",            "1.2345678901234567E-11x"));
		assert(test2("00000000000000000000.00000000012345678901234567",             "1.2345678901234568E-10x"));
		assert(test2("00000000000000000000.0000000012345678901234567",              "1.2345678901234566E-9x"));
		assert(test2("00000000000000000000.000000012345678901234567",               "1.2345678901234567E-8x"));
		assert(test2("00000000000000000000.00000012345678901234567",                "1.2345678901234566E-7x"));
		...
		assert(test2("00000012345678901234.56789010000000000000",      "1.2345678901234568E13x"));
		assert(test2("00000123456789012345.67890100000000000000",     "1.2345678901234567E14x"));
		assert(test2("00001234567890123456.78901000000000000000",    "1.2345678901234568E15x"));
		assert(test2("00012345678901234567.89010000000000000000",   "1.2345678901234568E16x"));
		assert(test2("00123456789012345678.90100000000000000000",  "1.2345678901234568E17x"));
#ifdef USE_TO_CHARS
...
#else
		assert(test2("01234567890123456789.01000000000000000000", "1.2345678901234568E18x"));
		assert(test2("12345678901234567890.10000000000000000000","1.2345678901234567E19x"));
#endif
...
//		assert(test2("00000000001234567890.12345678901000000000",          "1234567890.1234567x"));
	...
	assert(var(1e-11f).toString() == "9.999999960041972E-12");
	printl(var(1e-11));

#if defined(USE_RYU_D2S) or defined(USE_TO_CHARS)
	assert(var(1e-11).toString() == "1E-11");//ryu
#else
	...
	TRACE(var(1e-12f).toString())
	assert(var(1e-12f).toString() == "9.999999960041972E-13");
	assert(var(1e-12 ).toString() == "1E-12");
	assert(var(1e-13f).toString() == "9.9999998245167E-14");
	assert(var(1e-13 ).toString() == "1E-13");
	assert(var(1e-14f).toString() == "9.9999998245167E-15");
	assert(var(1e-14 ).toString() == "1E-14");
	...
	assert((var(pwr(10,-26)) ^ "x") =="1E-26x");//better calculation of 10^-26
	assert((var(1/pwr(10,26)) ^ "x") =="9.999999999999999E-27x");//poorer calculation of 1/10^26

	printl(((var(pwr(10,-26)) - var(1/pwr(10,26))) ^ "x"));
#if defined(USE_RYU_D2S) or defined(USE_TO_CHARS)
	assert(((var(pwr(10,-26)) - var(1/pwr(10,26))) ^ "x") == "1.4349296274686127E-42x");//ryu
#else
	...
	TRACE(var(pwr(10,-26)) ^ "x")
	TRACE(var(1/pwr(10,26)) ^ "x")
	TRACE(var(pwr(10,-27)) ^ "x")

	assert((var(pwr(10,-26)) ^ "x")  == "1E-26x");
	assert((var(1/pwr(10,26)) ^ "x") == "9.999999999999999E-27x");
	assert((var(pwr(10,-27)) ^ "x")  == "1E-27x");
...
#else
		assert(var(0.000'000'01) .toString() == "1e-08");
#endif
