...
function main() {
	...
	//			char* cp1 = nullptr;
				...
				break; case 0:
					if (repeatn == 0) printl("Exp: 35     ns - old var method - for (var;;)");
					nn = 10'000'000;
					for (var v2 = 0; v2 <= nn; v2++)
						{i1 = v2;};

				break; case 1:
					if (repeatn == 0) printl("Exp: 15     ns - new var method - for (var:range)");
					for (const var v2 : range(0 to nn))
						{i1 = v2;};

				break; case 2:
					if (repeatn == 0) printl("Exp: 0.840 ns - old int method - for (int;;)");
					for (int i2 = 0; i2 <= nn; i2++)
						{i1 = i2;};

				break; case 3:
					if (repeatn == 0) printl("Exp: 0.840 ns - new int method - for (int:range)");
					for (int i2 : range(0 to nn))
						{i1 = i2;};

				break; case 4:
					if (repeatn == 0) printl("Exp: 0.840 ns - new int method using boost - for (int:range)");
					for (int i2 : boost::irange(0, nn))
						{i1 = i2;};

				break; case 5:
					if (repeatn == 0) printl("Exp: 6.5   ns - construct empty var + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{var x; b1 = x.assigned();};

				break; case 6:
					if (repeatn == 0) printl("Exp: 6.5   ns - construct from int + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{var i = 123456; b1 = i.assigned();};

				break; case 7:
					if (repeatn == 0) printl("Exp: 6.5   ns - construct from bool + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{var b = true; b1 = b.assigned();};

				break; case 8:
					if (repeatn == 0) printl("Exp: 6.5   ns - construct from double + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{var d = 123.456; b1 = d.assigned();};

				break; case 9:
					if (repeatn == 0) printl("Exp: 12    ns - construct from cstr + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{var s = "1"; b1 = s.assigned();};

				break; case 10:
					if (repeatn == 0) printl("Exp: 12    ns - construct from char + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{var s = '1'; b1 = s.assigned();};

				break; case 11:
					if (repeatn == 0) printl("Exp: 12    ns - construct from std::string + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{var s = ss1; b1 = s.assigned();};

				break; case 12:
					if (repeatn == 0) printl("Exp: 4.5   ns - pure test");
					for (int i2 = 0; i2 <= nn; i2++)
						{b1 = v1.assigned();};

				break; case 13:
					if (repeatn == 0) printl("Exp: 0.9   ns - assign from int");
					for (int i2 = 0; i2 <= nn; i2++)
						{v1 = 123456;};

				break; case 14:
					if (repeatn == 0) printl("Exp: 0.9   ns - assign from bool");
					for (int i2 = 0; i2 <= nn; i2++)
						{v1 = true;};

				break; case 15:
					if (repeatn == 0) printl("Exp: 1.3   ns - assign from double");
					for (int i2 = 0; i2 <= nn; i2++)
						{v1 = 123.456;};

				break; case 16:
					if (repeatn == 0) printl("Exp: 13    ns - assign from cstr");
					for (int i2 = 0; i2 <= nn; i2++)
						{v1 = "1";};

				break; case 17:
					if (repeatn == 0) printl("Exp: 11    ns - assign from char");
					for (int i2 = 0; i2 <= nn; i2++)
						{v1 = '1';};

				break; case 18:
					if (repeatn == 0) printl("Exp: 11    ns - assign from std::string");
					for (int i2 = 0; i2 <= nn; i2++)
						{v1 = ss1;};

				break; case 19:
					if (repeatn == 0) printl("Exp: 22    ns - var + int + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{b1 = (v1 + i2).assigned();};

				break; case 20:
					if (repeatn == 0) printl("Exp: 22    ns - var + 0.1 + test");
					for (int i2 = 0; i2 <= nn; i2++)
						{b1 = (v1 + 0.1).assigned();};
