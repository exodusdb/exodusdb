...
////////////
		...
		if (not field(filename, OSSLASH, -1).contains("."))
			filename ^= ".cpp";
...
//			}
			...
			if (osfile(headerfilename)) {
				...
				let filename2 = headerline1.field(" ", -1).field("\"", 1);
				if (osfile(filename2))
					filename = filename2;
			}
		}
	...
	if (OSSLASH == "\\") {
		...
		if (not osfile(nanorcfilename)) {
			...
			if (oscopy(nanorctemplatefilename, nanorcfilename)) {
				...
				if (not var().input("Note: nano c++ syntax highlighting has been installed. Press Enter ... ")) {}
			} else {
