...
bool output_to_orig_dir = OPTIONS.errputl("OPTIONS=").contains("o");
...
//		auto deprecated = false;
		...
		for (var lineno : range(1, src.rows())) {
...
///////////
	...
	//			doc_body.replacer("^([a-zA-Z0-9_.]+): "_rex, R"__(\\fI$1:\\fR )__");
				...
				// .RE REturn (left shift)
				// .fi formatting
				doc_body.replacer(R"__(^<pre>)__"_rex, R"__(.nf)__");
				doc_body.replacer(R"__(^</pre>)__"_rex, R"__(.fi)__");
			...
			if (not output_to_orig_dir)
				...
				if (doc_text ne old_doc_text) {
					if (not oswrite(doc_text on doc_osfilename))
						abort(lasterror());
					outputl("gendoc: Updated; ", doc_osfilename);
				} else
