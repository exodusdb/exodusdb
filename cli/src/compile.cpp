	...
	*/
			...
			//if (headertext) {
			{
				...
				if (abs_srcfilename[1] != OSSLASH)
					abs_srcfilename.paster(1, 0, oscwd() ^ OSSLASH);
				var filebaseend = filebase.field2(OSSLASH, -1);
				headertext.paster(1, 0, "#define EXODUSDLFUNC_" ^ ucase(filebaseend) ^ "_H");
				headertext.paster(1, 0, OSSLASH ^ OSSLASH ^ "#ifndef EXODUSDLFUNC_" ^ ucase(filebaseend) ^ "_H" ^ crlf);
				headertext.paster(1, 0, OSSLASH ^ OSSLASH ^ "generated by exodus \"compile " ^ abs_srcfilename ^ DQ ^ crlf);
				//headertext.paster(1,0,OSSLASH^OSSLASH^"generated by exodus \"compile "^field2(filebase,OSSLASH,-1)^DQ^crlf);
...
function static compile2(
	...
	if (deftext) {
		deftext.paster(1, 0, "LIBRARY " ^ filebase ^ crlf ^ "EXPORTS");
