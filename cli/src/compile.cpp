...
//				var line = trimf(text2(ln));
	...
	*/
			...
			//if (headertext) {
			{
				...
				headertext.prefixer("#define EXODUSDLFUNC_" ^ ucase(filebaseend) ^ "_H");
				headertext.prefixer(OSSLASH ^ OSSLASH ^ "#ifndef EXODUSDLFUNC_" ^ ucase(filebaseend) ^ "_H" ^ crlf);
				headertext.prefixer(OSSLASH ^ OSSLASH ^ "generated by exodus \"compile " ^ abs_srcfilename ^ DQ ^ crlf);
