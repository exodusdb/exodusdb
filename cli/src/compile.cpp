...
"#pragma GCC diagnostic pop");
		...
		if (headertext) {
			...
			headertext.splicer(1,0,SLASH^SLASH^"generated by exodus \"compile "^field2(filebase,SLASH,-1)^DQ^crlf);
			headertext^=crlf^"//#endif"^crlf;
			//var headerfilename=filebase^".h";
			var headerfilename=incdir^SLASH^filebase^".h";

			//create include directory if doesnt already exist
			if (not incdir.osdir()) {
				if (not incdir.osmkdir()) {
					printl("Error: exodus compile could not create dir for " ^ headerfilename.squote() ^ ". Skipping creation of header file" );
				} else if (verbose)
			...
			if (headertext2 ne headertext) {
				...
				osread(headertext2,headerfilename,locale);
				if (headertext2 ne headertext)
					printl("Error: compile could not accurately update " ^ headerfilename ^ " locale " ^ locale);
				else if (verbose)
