...
//subfiles.outputl("subfiles2=");
		...
		if (not(osfile(srcfilename)) and not(srcfilename.index(SLASH))) {
			var headerfilename=osgetenv("HOME") ^ SLASH ^ "inc" ^ SLASH ^ srcfilename;
...
"#pragma GCC diagnostic pop");
		...
		//if (headertext) {
		{
			...
			headertext.splicer(1,0,SLASH^SLASH^"generated by exodus \"compile "^abs_srcfilename^DQ^crlf);
			...
			var headerfilename=incdir^SLASH^filebaseend^".h";
