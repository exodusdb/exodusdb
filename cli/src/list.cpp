...
class list
{
	...
	//all "variables" that are to be commonly available to all local functions and subroutines (class methods)
	//must be "declared" (given names) at the class level but they cannot be initialised here - unless made "static".
...
//	var icol;
	...
	var coldict[maxncols2];//TODO:var coldict(maxncols);
	var mcol[maxncols2];//TODO:var mcol(maxncols);
	var pcol[maxncols2];//TODO:var pcol(maxncols);
	var ccol[maxncols2];//TODO:var ccol(maxncols);
	var scol[maxncols2];//TODO:var scol(maxncols);
	var icol[maxncols2];//TODO:var icol(maxncols);
	var wcol[maxncols2];//TODO:var icol(maxncols);

	varray breakcount;		//TODO:var breakcount(nbreaks + 1);
	varray breaktotal;		//TODO:var breaktotal(maxncols, nbreaks + 1);

	varray breakvalue;//(maxncols);	
	varray oldbreakvalue;//(maxncols);
...
list(MvEnvironment& env_) : env(env_)
{
	quotes=DQ^SQ;
...
*/
	...
	if (env.USER2.substr(2, 1) eq "C") {
		var decimalchar = ",";
	}else{
		var decimalchar = ".";
		}

	//if (env.SYSTEM.extract(2) eq "") {
	//	perf("GET NEW " ^ env.SENTENCE);
	//	var().stop();
	//}
	...
	if (html) {
		...
		tdx = "</td>";
		nbsp = "&nbsp;";
		tt = env.SYSTEM.extract(2);
		tt.swapper(".txt", ".htm");
		env.SYSTEM.replacer(2, 0, 0, tt);
	}else{
		tr = "";
		trx = crlf;
		td = "";
...
////////
	...
	var thcolor = env.SYSTEM.extract(46, 1);
	var tdcolor = env.SYSTEM.extract(46, 2);
	var reportfont = env.SYSTEM.extract(46, 3);
	if (!tdcolor)
		tdcolor = "#FFFFC0";
	if (!thcolor)
		thcolor = "#FFFF80";
...
//filename:
		...
		/*
		if (var("PLANS,SCHEDULES,ADS,BOOKING.ORDERS,VEHICLES,RATECARDS").locateusing(filename, ",", xx))
			printtxmark = "Media Management";

		if (var("JOBS,PRODUCTION.ORDERS,PRODUCTION.INVOICES").locateusing(filename, ",", xx))
...
///////
	...
	if (!(coln or crtx)) {
		word = "@LPTR";
		if (env.DICT and not xx.read(env.DICT, word)) {
			word = "@CRT";
			if (not xx.read(env.DICT, word))
				word = "";
		}
		if (word) {
			crtx = 1;
	...
	if (!idsupp) {
		...
		//set column 1 @ID
		colname[1] = "@" "ID";
		if (not env.DICT or not coldict[1].read(env.DICT, "@" "ID")) {
...
////////
	...
	if (!(openfile(filename, srcfile)))
		stop("Cannot open file " ^ filename);

	breakcount.resize(nbreaks + 1);		//TODO:var breakcount(nbreaks + 1);
	breaktotal.resize(maxncols, nbreaks+1);	//TODO:var breaktotal(maxncols, nbreaks + 1);
	breakvalue.resize(maxncols);		//TODO:var breakvalue(maxncols);
	oldbreakvalue.resize(maxncols);		//TODO:var oldbreakvalue(maxncols);
...
//////////////////////////////
{
	...
	while (true)
	{
		...
		env.FILEERRORMODE = 1;
		env.FILEERROR = "";
		if (not filename.readnext(env.ID, env.MV)) {

			env.FILEERRORMODE = 0;
			/* treat all errors as just "no more records"
			if (_STATUS) {
				tx = "*** Fatal Error " ^ env.FILEERROR.extract(1) ^ " reading record " ^ env.ID ^ " ***";
				printer1.printtx(tx);
				var().stop();
			}
			if (env.FILEERROR.extract(1) eq 421) {
				tx = "Operation aborted by user.";
				printer1.printtx(tx);
				var().stop();
			}
			if (env.FILEERROR and env.FILEERROR.extract(1) not_eq 111) {
				tx = "*** Error " ^ env.FILEERROR.extract(1) ^ " reading record " ^ env.ID ^ " ***";
				printer1.printtx(tx);
				readerr += 1;
				var().stop();
			}
			...
			for (var limitn = 1; limitn <= nlimits; limitn++) {
				...
				for (var varx = nvars; varx >= 1; varx--) {
					...
					if (!(limits.extract(3, limitn).locateusing(temp, SVM, xx))) {
						for (var fn = 1; fn <= nfns; fn++) {
							var varalues = env.RECORD.extract(fn);
							if (varalues.count(VM))
								env.RECORD.eraser(fn, varx, 0);
						};//fn;
					}
				};//varx;
			};//limitn;
...
///////////////////////////////
{
	...
	//var().getcursor();
	//if (not env.SYSTEM.extract(33)) {
	//	print(env.AW.extract(30), var().cursor(36, env.CRTHIGH / 2));
	//}else{
	//	print(var().cursor(0));
	//}
	//print(recn, " ");
		...
		//}else{
			...
			if (dictrec.extract(1) eq "F") {
				...
				if (not fieldno) {
					...
					cell=env.RECORD.extract(fieldno);
			} else {
				cell="";
			}
...
////////////////////////
{
	...
	//standardise
	if (env.DICT ne "" and dictrec.read(env.DICT, word)) {
...
/////////
{
	//this will all be done automatically by exodus soon
