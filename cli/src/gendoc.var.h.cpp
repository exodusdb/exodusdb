//Generated by cli/gendoc from var.h 05 FEB 2025 16:06:36
...
function main() {
	...
	{
		if ("UK]US]UA"_var.locate("US")) {/*ok*/} else  abort("locate: " ^ lasterror());
		// or
	...
	{
		var valueno;
		if ("UK]US]UA"_var.locate("US", valueno)) {/*ok*/} else  abort("locate: " ^ lasterror());  assert(valueno == 2);
	}
	...
	{
		var entries1 = "/etc/"_var.oslist("*.cfg"); /// e.g. "adduser.conf^ca-certificates.conf^ abort("oslist: " ^ lasterror());"
		// or
