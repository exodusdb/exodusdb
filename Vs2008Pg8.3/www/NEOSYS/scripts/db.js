...
var trailingspaces=/\s*$/gi

function neosys_dict_emailaddress(di)
{
 neosys_dict_text(di,20)
 di.validation='neosys_val_emailaddress()'
}

function neosys_val_emailaddress()
{
...
// if (!gvalue.match(/[-_.abcdefghijklmnopqrstuvwxyz1234567890]+@[-.abcdefghijklmnopqrsptuvwxyz1234567890]+\.[-_.abcdefghijklmnopqrstuvwxyz]+/i))
 if (gvalue&&!gvalue.match(/^[\w.-]+@[^\. -_][\w.-]*\.[a-z]{2,}$/))
 {
  return neosysinvalid('Email address must be in the format xxxxxx@xxxxxx.xxx')
 }
 return true
}
...
function neosys_def_time(mode,othertimeid)
{
 ...
 othertime=gds.calcx(othertimeid)
...
function neosys_val_time(mode,otherid)
{
 ...
 var othertime=gds.calcx(otherid)
 if (typeof othertime=='object') othertime=othertime[0]
 ...
 if (othertime2!=(+othertime)) gds.setrecord(otherid,grecn,othertime.toString())
...
neosysrecord.prototype.postread=function neosysrecord_postread()
{
 ...
 {
  this.key=response[temp].replace(/\{20\}/gi,' ')
  response[temp-1]=''
  response[temp]=''
 }
      ...
      with (this.dict[dictn])
      {
       if (groupno==groupn)
       {
        ...
        else
        {
         ...
         //mac if (groupn==0) temp.replace(/\xFC/gi,'\r')
         temp=temp.replace(tms,'\r')
 ...
 neosysrecord.prototype.obj2rev=function neosysrecord_obj2rev()
 {
  ...
  for (var groupn=0;groupn<=ngroups;groupn++)
  {
   ...
   for (var valuen=0;valuen<nvalues;valuen++)
   {
    ...
    for (var datafld in record)
    {
     
     //locate the data item
     var child=record[datafld]
     
     //locate dictionary id then process
     if (dictitem=this.dictitem(datafld))
     {
      with (dictitem)
      {
       ...
       if (groupno==groupn)
       {
        ...
        if (temp||typeof wordno!='undefined')
        {
         ...
         temp=temp.replace(/\r/gi,'\xFB').replace(/\n/gi,'')
...
// if (typeof typeofcode0!='undefined')
 ...
 if (typeof(conversion7)=='undefined') conversion7=''
 if (typeof(functioncode8)=='undefined') functioncode8=''
 if (typeof(align9)=='undefined') align9=''
 if (typeof(length10)=='undefined') length10=''
