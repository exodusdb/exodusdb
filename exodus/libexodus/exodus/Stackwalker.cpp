 ...
 *most Lint (http://www.gimpel.com/) errors... thanks to Okko Willeboordse!
 *
 **********************************************************************/
#ifdef _MSC_VER
#include <stdio.h>
...
// Avoid warning C4748: /GS can not protect parameters and local variables from local buffer overrun
// because optimizations are disabled
...
class StackWalkerInternal {
   public:
	...
	BOOL Init(LPCSTR szSymPath) {
		...
		if (GetModuleFileName(nullptr, szTemp, 4096) > 0) {
			...
			if (GetFileAttributes(szTemp) == INVALID_FILE_ATTRIBUTES) {
				...
				if ((m_hDbhHelp == nullptr) &&
					...
					_tcscat_s(szTemp,
							  _T("\\Debugging Tools for Windows ")
							  _T("64-Bit\\dbghelp.dll"));
					if (GetFileAttributes(szTemp) != INVALID_FILE_ATTRIBUTES) {
						m_hDbhHelp = LoadLibrary(szTemp);
					}
				}
		...
		if (pSC == nullptr || pSFTA == nullptr || pSGMB == nullptr || pSGMI == nullptr ||
			pSGO == nullptr || pSGSFA == nullptr || pSI == nullptr || pSSO == nullptr || pSW == nullptr ||
			pUDSN == nullptr || pSLM == nullptr) {
			FreeLibrary(m_hDbhHelp);
			m_hDbhHelp = nullptr;
			pSC = nullptr;
			return FALSE;
	...
	*/
	...
	//  // SymGetModuleInfo64()
	//  typedef BOOL (__stdcall *tSGMI_V3)(IN HANDLE hProcess, IN DWORD64 dwAddr, OUT
	//  IMAGEHLP_MODULE64_V3 *ModuleInfo ); tSGMI_V3 pSGMI_V3;
...
cleanup:
		...
		if ((m_parent != nullptr) && (szImg != nullptr)) {
			...
			if ((this->m_parent->m_options & StackWalker::RetrieveFileVersion) != 0) {
				...
				if (dwSize > 0) {
					...
					if (vData != nullptr) {
						if (GetFileVersionInfoA(szImg, dwHandle, dwSize,
												vData) != 0) {
							UINT len;
							TCHAR szSubBlock[] = _T("\\");
							if (VerQueryValue(vData, szSubBlock,
											  (LPVOID*)&fInfo,
											  &len) == 0)
								fInfo = nullptr;
							else {
			...
			this->m_parent->OnLoadModule(img, mod, baseAddr, size, result, szSymType,
										 Module.LoadedImageName, fileVersion);
		}
		if (szImg != nullptr)
			free(szImg);
		if (szMod != nullptr)
   ...
   public:
	...
	BOOL GetModuleInfo(HANDLE hProcess, DWORD64 baseAddr, IMAGEHLP_MODULE64_V2* pModuleInfo) {
		...
		//        return FALSE;
		//    }
		// could not retrive the bigger structure, try with the smaller one (as defined in
		// VC7.1)...
		pModuleInfo->SizeOfStruct = sizeof(IMAGEHLP_MODULE64_V2);
		void* pData = malloc(4096);	 // reserve enough memory, so the bug in v6.3.5.1 does
									 // not lead to memory-overwrites...
		if (pData == nullptr) {
	...
	if ((this->m_options & SymBuildPath) != 0) {
		...
		if ((this->m_options & SymBuildPath) != 0) {
			if (GetEnvironmentVariableA("SYSTEMDRIVE", szTemp, nTempLen) > 0) {
				...
				strcat_s(szSymPath, nSymPathLen,
						 "SRV*c:\\websymbols*http://msdl.microsoft.com/download/"
						 "symbols;");
		}
	}
...
#error "Platform not supported!"
	...
	for (frameNum = 0;; ++frameNum) {
		...
		// ERROR_NOACCESS (998), you can assume that either you are done, or that the stack
		// is so hosed that the next deeper frame could not be found. CONTEXT need not to be
		// suplied if imageTyp is IMAGE_FILE_MACHINE_I386!
...
BOOL __stdcall StackWalker::myReadProcMem(HANDLE hProcess, DWORD64 qwBaseAddress, PVOID lpBuffer, DWORD nSize, LPDWORD lpNumberOfBytesRead) {
	if (s_readMemoryFunction == nullptr) {
		SIZE_T st;
		BOOL bRet =
			ReadProcessMemory(hProcess, (LPVOID)qwBaseAddress, lpBuffer, nSize, &st);
		*lpNumberOfBytesRead = (DWORD)st;
		// printf("ReadMemory: hProcess: %p, baseAddr: %p, buffer: %p, size: %d, read: %d,
...
void StackWalker::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName, ULONGLONG fileVersion) {
	...
	if (fileVersion == 0)
		...
		_snprintf_s(buffer, STACKWALK_MAX_NAMELEN,
					"%s:%s (%p), size: %d (result: %d), SymType: '%s', PDB: '%s', "
					"fileVersion: %d.%d.%d.%d\n",
					img, mod, (LPVOID)baseAddr, size, result, symType, pdbName, v1, v2, v3,
					v4);
	}
	OnOutput(buffer);
}
