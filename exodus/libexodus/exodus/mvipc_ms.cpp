...
int MVipc(const int environmentn, var& pgconnparams) {
	...
	for (i = 0; i < INSTANCES; i++) {
		...
		if (Pipe[i].hPipeInst == INVALID_HANDLE_VALUE) {
			if (GetLastError() == 231) {
				wprintf(
					L"MVipc() CreateNamedPipe failed because pipe %s is busy. "
	...
	while (1) {
			...
			switch (Pipe[i].dwState) {
				...
				case WRITING_STATE:
					...
					if (!cbRet != Pipe[i].cbToWrite) {
						wprintf(
							L"MVipc() WRITING_STATE but cbRet (%d) is not "
							L".cbToWrite (%d)\n",
		...
		switch (Pipe[i].dwState) {
...
#if TRACING >= 3
					wprintf(
						L"MVipc() READING_STATE but !fSuccess and the pipe has "
						L"been ended.\n");
#endif
			...
			*/
				...
				// An error occurred; disconnect from the client.
				wprintf(
					L"MVipc() An error occurred; disconnect and reconnect to the "
					L"client.\n");
				DisconnectAndReconnect(i);
				break;
