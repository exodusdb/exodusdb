...
function neosysamountinwords(amount)
{
 ...
 var text='ýZeroýOneýTwoýThreeýFourýFiveýSixýSevenýEightýNineýTenýElevenýTwelveýThirteenýFourteenýFifteenýSixteenýSeventeenýEighteenýNineteenýTwentyýThirtyýFortyýFiftyýSixtyýSeventyýEightyýNinety'.split(vm)
 if (+units)
 {
  if (words) words+=' and '
...
String.prototype.neosyscapitalise=function neosyscapitalise() 
{
 ...
 //return string.toLowerCase().replace(/(\b[a-z])/gi,function($1){return $1.toUpperCase()})
...
//function neosysadd()
Array.prototype.neosysadd=function(array2)
{
 var results=[]
 for (var i=0;i<this.length;i++)
 {
  //results[i]=this[i]+array2[i]
  results[i]=+this[i]+(Number(typeof array2=='object'?array2[i]:array2))
 }
 return results
}
...
Array.prototype.neosysmultiply=function(array2)
{
 ...
 for (var i=0;i<this.length;i++)
 {
  var multiplier=typeof array2=='object'?array2[i]:array2
  if (this[i]) results[i]=this[i]*Number(multiplier)
  else results[i]=this[i]
 }
 return results
}
...
function arrayneosysdivide(array2,divzero)
{
 ...
 for (var i=0;i<this.length;i++)
 {
  var divisor=(Number(typeof array2=='object'?array2[i]:array2))
  if (+this[i])
  {
   if (!divisor)
   {
...
String.prototype.neosyssum=function(sepchar)
{
 
 //convert multivalued string to array
 if (typeof sepchar=='undefined') sepchar=vm//this should be ',' really
 if (this.indexOf(sepchar)>=0) return this.split(sepchar).neosyssum(sepchar)
...
Array.prototype.neosyssum=function neosyssumarray(sepchar)
{
 ...
 var total=0
 for (var i=0;i<this.length;i++) total+=this[i].neosyssum(sepchar)

 if (Math.abs(total)<0.0000000001) total=0
...
//check if numeric
gnumericregexp=RegExp('(^[-+]?\\d*[.]?\\d+$)|(^[-+]?\\d+[.]?\\d*$)|(^$)')
function neosysnum(arg1)
{ 
 try
 {
  if (typeof arg1=='number') return true 
  return arg1.toString().match(gnumericregexp)!=null
 }
 catch (e)
 {
  return false
...
function neosysconvarray(functionx,mode,value,params)
{
 ...
 {
  result[i]=functionx(mode,value[i],params)
 ...
 function NUMBER(mode,value,params)
 {
  ...
  if (typeof(value)=='object') return neosysconvarray(NUMBER,mode,value,params)

  if (typeof value=='string')
  {

   //can handle records, multivalues and subvalues
   if (value.indexOf(fm)+1) {return neosysconvarray(NUMBER,mode,value.split(fm),params).join(fm)}
   else if (value.indexOf(vm)>+1) return neosysconvarray(NUMBER,mode,value.split(vm),params).join(vm)
   else if (value.indexOf(sm)>+1) return neosysconvarray(NUMBER,mode,value.split(sm),params).join(sm)
  ...
  if (params[0]=='') params[0]=value.toString().neosysfield('.',2,1).length.toString()
...
//   var result=parseFloat(value)
 ...
 {
   return 'e.g. 31, 31/1, 31/1/01 or 31/1/2001'
 }
 ...
 function DATE(mode,value,params)
 {
  ...
  if (mode=='ICONV')
  {
   ...
   //four or five digits are assumed to be 
   if (value.match(/^\d{4,5}$/)) return value
   
   //assume in the format 31x1x2000
   value=value.replace(/\W/g,' ').split(' ')

   //check day is one or two digits
   if (!value[0].match(/^\d{1,2}$/))
   {
    gmsg=invaliddatemsg()
    return null
   }
   ...
   if (params.indexOf('=')+1)
   {
    ...
    otherdate=gds.calcx(otherdateid,grecn)
   ...
   if (!value[1]) value[1]=new Date().getMonth()+1

   //default to year parameter
   if (!value[2]&&!!params[1]) value[2]=params[1]

   //default to current year
   if (value[2])
    value[2]=ADDCENT(value[2])
   else
    value[2]=new Date().getFullYear()
   
   //check integers
   if (isNaN(parseInt(value[0],10))||isNaN(parseInt(value[1],10))||isNaN(parseInt(value[2],10)))
   {
    gmsg=invaliddatemsg()
    return null
   }
...
*/
   ...
   var result=(Date.UTC(value[2],value[1]-1,value[0])-Date.UTC(1967,11,31))/24/60/60/1000
   ...
   if (parseInt(result2[0],10)!=parseInt(value[0],10)||parseInt(result2[1],10)!=parseInt(value[1],10))
   {
    gmsg=invaliddatemsg()
    return null
   }
   ...
   if (fromto)
   {
    ...
    if (otherdate!=otherdate0) gds.setrecord(otherdateid,grecn,otherdate)
...
//  if (value=='"') alert('x'+value)
   ...
   */
   ...
   switch (params)
   {
    ...
    {
     //like default
     var result=result.getUTCDate()+'/'+(result.getUTCMonth()+1)+'/'+result.getUTCFullYear()
     var dow=((value-1)%7)+1
     result=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'][dow-1]+' '+result
     var curryear=(new Date().getFullYear()).toString()
     if (result.slice(-curryear.length)==curryear)
     {
      result=result.slice(0,-curryear.length-1)
      var currmonth=(new Date().getMonth()+1).toString()
      if (result.slice(-currmonth.length+1)==currmonth)
      {
       result=result.slice(0,-currmonth.length-1)
      }
     }
     break
    }
 ...
 function TIME(mode,value,params)
 {
  ...
  gmsg='e.g. 00:00, 23:59, 11:59:59, 24:00, 25:00'
 ...
 else
 {
  ...
  //format must be HH:MM or HH:MM:SS
  var temp=value.neosysconvert('. ','::').split(':')
  if (!temp[1]) temp[1]='00'
...
function DATE_TIME(mode,value,params)
{
 ...
 if (mode=='OCONV')
 {
  ...
  if (!value[1]) value[1]=''
  var date=value[0].neosysoconv('[DATE,'+params[0]+']')
  var time=value[1].neosysoconv('[TIME,'+params[1]+']')
  result=date+' '+time
 }
 else
 {
...
function neosysaddunits(a,b)
{
 ...
 //add a to b
 //a and b can be a mv list or js array of amounts with unit codes eg 200STGý300YENý100USD
...
//replace
String.prototype.neosysreplace=neosysreplace

function neosysreplace(fieldno,valueno,subvalueno,replacestring)
{
...
String.prototype.neosysswap=function neosysswapstring(swapfrom,swapto)
{
 ...
 //make sure all characters are escaped
 //if (swapfrom) swapfrom='\\'+swapfrom.split('').join('\\')
 swapfrom=swapfrom.replace(/([\\,\^,\$,\*,\+,\?,\.,\(,\),\|,\{,\},\[,\]])/gi,"\\$1")
...
function YEAR_PERIOD(type,input,mode)
{
 ...
 if (type=='ICONV')
 {
  ...
  //check year ... ICONV converts to 2 digits, OCONV converts to 4 digits
  if (mode=='4') year=ADDCENT('OCONV',year,mode)
  else year=ADDCENT(type,year,mode)
  if (year==null) return null
