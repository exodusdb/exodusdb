...
// try
	{
		...
		{
			var datasets=getdatasets(this.neosysrootpath,requests[5])
		...
		if (typeof(this.portno)=='undefined'||this.dataset=='')
		{
...
*/
			//see if autostartdatabase should be suppressed
			if (gautostartdatabase)
			{
				...
				try
				{
					...
					file.Close()
					if (text.toUpperCase().indexOf('AUTOSTART=NO')<0) gautostartdatabase=false
				}
				catch(e)
				{
				}
...
//				Response.End()
			...
			{
				this.response=e.number+' "'+e.description+'" waiting4.exe not run or failed'
				dbready(dbwaitingwindow)
				this.request=''
				return 0
			}
...
//Response.End()
			...
			//autostarting database so check request has been heard
			if (gautostartdatabase)
			{
				try
				{
					tf=gfso.GetFile(linkfilename + '.1')
					this.response='Cannot start database at '+this.neosysrootpath
					dbready(dbwaitingwindow)
					this.request=''
					return 0
				}
				...
				{
					//ok request file gone/heard
				}
			}

/*
			while (gautostartdatabase)
			{
...
function getdatasets(neosysrootpath,systemcode)
{
	...
	//get the first line
	var datasetvolfilename=proglocation+systemcode+'.vol'
	var datasets=neosysosread(datasetvolfilename)
	if (datasets=='')
	{
		db.data=''
		db.response='Error: Dataset file '+datasetvolfilename+' is missing or cannot be accessed'
		dbready(dbwaitingwindow)
		db.request=''
		return 0
	}
	...
	//split by line
	datasets=datasets.split('\r')
	//or by field mark
	if (datasets.length==1) datasets=datasets[0].split(String.fromCharCode(254))
	//get the first line but strip off the firstword
	datasets=datasets[0].substr(datasets[0].indexOf(' ')+1)
	//split by '*'
	...
	{
		db.data=''
		db.response='Error: Dataset file '+datasetvolfilename+' does not contain any valid datasets'
		dbready(dbwaitingwindow)
		db.request=''
		return 0
	}
